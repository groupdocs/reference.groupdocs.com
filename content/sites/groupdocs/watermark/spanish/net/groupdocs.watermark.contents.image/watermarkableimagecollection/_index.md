---
title: WatermarkableImageCollection
second_title: Referencia de API de GroupDocs.Watermark para .NET
description: Representa una colección de imágenes encontradas en un documento.
type: docs
weight: 430
url: /es/net/groupdocs.watermark.contents.image/watermarkableimagecollection/
---
## WatermarkableImageCollection class

Representa una colección de imágenes encontradas en un documento.

```csharp
public class WatermarkableImageCollection : ReadOnlyListBase<WatermarkableImage>
```

## Propiedades

| Nombre | Descripción |
| --- | --- |
| virtual [Count](../../groupdocs.watermark.common/readonlylistbase-1/count) { get; } | Obtiene el número de elementos que contiene la colección. |
| virtual [IsReadOnly](../../groupdocs.watermark.common/readonlylistbase-1/isreadonly) { get; } | Obtiene un valor que indica si la colección es de solo lectura. |
| virtual [Item](../../groupdocs.watermark.common/readonlylistbase-1/item) { get; } | Obtiene el elemento en el índice especificado en la colección. |

## Métodos

| Nombre | Descripción |
| --- | --- |
| virtual [Contains](../../groupdocs.watermark.common/readonlylistbase-1/contains)(WatermarkableImage) |  |
| virtual [GetEnumerator](../../groupdocs.watermark.common/readonlylistbase-1/getenumerator)() |  |
| virtual [IndexOf](../../groupdocs.watermark.common/readonlylistbase-1/indexof)(WatermarkableImage) |  |

### Observaciones

Esta colección contiene los elementos de[`WatermarkableImage`](../watermarkableimage) tipo.

### Ver también

* class [ReadOnlyListBase&lt;T&gt;](../../groupdocs.watermark.common/readonlylistbase-1)
* class [WatermarkableImage](../watermarkableimage)
* espacio de nombres [GroupDocs.Watermark.Contents.Image](../../groupdocs.watermark.contents.image)
* asamblea [GroupDocs.Watermark](../../)

<!-- DO NOT EDIT: generated by xmldocmd for GroupDocs.Watermark.dll -->
