---
title: PresentationWatermarkMasterSlideOptions
second_title: .NET API 参考的 GroupDocs.Watermark
description: 表示向演示文档母版幻灯片添加水印时的选项
type: docs
weight: 2030
url: /zh/net/groupdocs.watermark.options.presentation/presentationwatermarkmasterslideoptions/
---
## PresentationWatermarkMasterSlideOptions class

表示向演示文档母版幻灯片添加水印时的选项。

```csharp
public sealed class PresentationWatermarkMasterSlideOptions : PresentationWatermarkBaseSlideOptions
```

## 构造函数

| 姓名 | 描述 |
| --- | --- |
| [PresentationWatermarkMasterSlideOptions](presentationwatermarkmasterslideoptions)() | 初始化一个新的实例[`PresentationWatermarkMasterSlideOptions`](../presentationwatermarkmasterslideoptions)类. |

## 特性

| 姓名 | 描述 |
| --- | --- |
| [AlternativeText](../../groupdocs.watermark.options.presentation/presentationwatermarkbaseslideoptions/alternativetext) { get; set; } | 获取或设置将与形状关联的描述性（替代）文本。 |
| [Effects](../../groupdocs.watermark.options.presentation/presentationwatermarkbaseslideoptions/effects) { get; set; } | 获取或设置一个值[`PresentationImageEffects`](../presentationimageeffects)or [`PresentationTextEffects`](../presentationtexteffects)对于应该应用于水印的效果。 |
| [IsLocked](../../groupdocs.watermark.options.presentation/presentationwatermarkbaseslideoptions/islocked) { get; set; } | 获取或设置一个值，该值指示是否禁止在 PowerPoint 中编辑形状。 |
| [MasterSlideIndex](../../groupdocs.watermark.options.presentation/presentationwatermarkmasterslideoptions/masterslideindex) { get; set; } | 获取或设置要添加水印的母版幻灯片的索引。 |
| [Name](../../groupdocs.watermark.options.presentation/presentationwatermarkbaseslideoptions/name) { get; set; } | 获取或设置形状的名称。 |
| [ProtectWithUnreadableCharacters](../../groupdocs.watermark.options.presentation/presentationwatermarkbaseslideoptions/protectwithunreadablecharacters) { get; set; } | 获取或设置文本水印字符是否混有不可读字符的值。 |

### 评论

**了解更多：**

* [向演示文档添加水印](https://docs.groupdocs.com/display/watermarknet/Add+watermarks+to+presentation+documents)

请参阅中的用法示例[`PresentationWatermarkBaseSlideOptions`](../presentationwatermarkbaseslideoptions)

### 也可以看看

* class [PresentationWatermarkBaseSlideOptions](../presentationwatermarkbaseslideoptions)
* 命名空间 [GroupDocs.Watermark.Options.Presentation](../../groupdocs.watermark.options.presentation)
* 部件 [GroupDocs.Watermark](../../)

<!-- DO NOT EDIT: generated by xmldocmd for GroupDocs.Watermark.dll -->
