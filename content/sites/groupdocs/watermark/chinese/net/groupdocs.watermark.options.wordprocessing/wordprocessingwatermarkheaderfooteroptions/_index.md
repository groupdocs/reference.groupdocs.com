---
title: WordProcessingWatermarkHeaderFooterOptions
second_title: .NET API 参考的 GroupDocs.Watermark
description: 表示将水印添加到 Word 部分页眉/页脚时的选项
type: docs
weight: 2350
url: /zh/net/groupdocs.watermark.options.wordprocessing/wordprocessingwatermarkheaderfooteroptions/
---
## WordProcessingWatermarkHeaderFooterOptions class

表示将水印添加到 Word 部分页眉/页脚时的选项。

```csharp
public sealed class WordProcessingWatermarkHeaderFooterOptions : WordProcessingWatermarkBaseOptions
```

## 构造函数

| 姓名 | 描述 |
| --- | --- |
| [WordProcessingWatermarkHeaderFooterOptions](wordprocessingwatermarkheaderfooteroptions)() | 初始化[`WordProcessingWatermarkHeaderFooterOptions`](../wordprocessingwatermarkheaderfooteroptions)类. |

## 特性

| 姓名 | 描述 |
| --- | --- |
| [AlternativeText](../../groupdocs.watermark.options.wordprocessing/wordprocessingwatermarkbaseoptions/alternativetext) { get; set; } | 获取或设置将与形状关联的描述性（替代）文本。 |
| [Effects](../../groupdocs.watermark.options.wordprocessing/wordprocessingwatermarkbaseoptions/effects) { get; set; } | 获取或设置一个值[`WordProcessingImageEffects`](../wordprocessingimageeffects)or [`WordProcessingTextEffects`](../wordprocessingtexteffects)对于应该应用于水印的效果。 |
| [HeaderFooterType](../../groupdocs.watermark.options.wordprocessing/wordprocessingwatermarkheaderfooteroptions/headerfootertype) { get; set; } | 获取或设置标识要添加水印的页眉或页脚类型的值。 |
| [IsLocked](../../groupdocs.watermark.options.wordprocessing/wordprocessingwatermarkbaseoptions/islocked) { get; set; } | 获取或设置一个值，该值指示是否禁止在 Word 中编辑形状。 |
| [LockType](../../groupdocs.watermark.options.wordprocessing/wordprocessingwatermarkbaseoptions/locktype) { get; set; } | 获取或设置水印锁定类型。 |
| [Name](../../groupdocs.watermark.options.wordprocessing/wordprocessingwatermarkbaseoptions/name) { get; set; } | 获取或设置形状的名称。 |
| [Password](../../groupdocs.watermark.options.wordprocessing/wordprocessingwatermarkbaseoptions/password) { get; set; } | 获取或设置用于锁定水印的密码。 |
| [SectionIndex](../../groupdocs.watermark.options.wordprocessing/wordprocessingwatermarkheaderfooteroptions/sectionindex) { get; set; } | 获取或设置要添加水印的节的索引。 |

### 也可以看看

* class [WordProcessingWatermarkBaseOptions](../wordprocessingwatermarkbaseoptions)
* 命名空间 [GroupDocs.Watermark.Options.WordProcessing](../../groupdocs.watermark.options.wordprocessing)
* 部件 [GroupDocs.Watermark](../../)

<!-- DO NOT EDIT: generated by xmldocmd for GroupDocs.Watermark.dll -->
