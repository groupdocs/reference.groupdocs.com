---
title: DiagramLoadOptions class
second_title: GroupDocs.Watermark for Python via .NET API References
description: 
type: docs
url: /python-net/groupdocs.watermark.options.diagram/diagramloadoptions/
is_root: false
weight: 10
---

## DiagramLoadOptions class

Represents document loading options for a Visio document.



**Inheritance:** [`DiagramLoadOptions`](/watermark/python-net/groupdocs.watermark.options.diagram/diagramloadoptions) → 
[`LoadOptions`](/watermark/python-net/groupdocs.watermark.options/loadoptions)



The DiagramLoadOptions type exposes the following members:

### Constructors
| Constructor | Description |
| :- | :- |
| [__init__](/watermark/python-net/groupdocs.watermark.options.diagram/diagramloadoptions/__init__/#) | Initializes a new instance of the [`DiagramLoadOptions`](/watermark/python-net/groupdocs.watermark.options.diagram/diagramloadoptions) class. |
| [__init__](/watermark/python-net/groupdocs.watermark.options.diagram/diagramloadoptions/__init__/#System.String) | Initializes a new instance of the [`DiagramLoadOptions`](/watermark/python-net/groupdocs.watermark.options.diagram/diagramloadoptions) class with a specified password. |


### Properties
| Property | Description |
| :- | :- |
| [password](/watermark/python-net/groupdocs.watermark.options.diagram/diagramloadoptions/password) | Gets or sets the password for opening an encrypted document. |
| [format_family](/watermark/python-net/groupdocs.watermark.options.diagram/diagramloadoptions/format_family) | Gets or sets the format family of the document, <br/>indicating its type (e.g., Image, Pdf, Spreadsheet, etc.). |
| [file_type](/watermark/python-net/groupdocs.watermark.options.diagram/diagramloadoptions/file_type) | Gets or sets the type of the file, <br/>indicating its type (e.g., docx, pdf, xlsx, etc.). |
| [default](/watermark/python-net/groupdocs.watermark.options.diagram/diagramloadoptions/default) | Gets the default value for the [`DiagramLoadOptions`](/watermark/python-net/groupdocs.watermark.options.diagram/diagramloadoptions) class. |



### See Also
* module [`groupdocs.watermark.options.diagram`](..)
* class [`DiagramLoadOptions`](/watermark/python-net/groupdocs.watermark.options.diagram/diagramloadoptions)
* class [`LoadOptions`](/watermark/python-net/groupdocs.watermark.options/loadoptions)
