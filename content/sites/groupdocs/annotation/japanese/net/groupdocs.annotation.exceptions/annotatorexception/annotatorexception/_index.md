---
title: AnnotatorException
second_title: GroupDocs.Annotation for .NET API リファレンス
description: の新しいインスタンスを初期化しますAnnotatorExceptiongroupdocs.annotation.exceptions/annotatorexceptionclass.
type: docs
weight: 10
url: /ja/net/groupdocs.annotation.exceptions/annotatorexception/annotatorexception/
---
## AnnotatorException(string) {#constructor}

の新しいインスタンスを初期化します[`AnnotatorException`](../../annotatorexception)class.

```csharp
public AnnotatorException(string message)
```

| パラメータ | タイプ | 説明 |
| --- | --- | --- |
| message | String | エラーを説明するメッセージ。 |

### 関連項目

* class [AnnotatorException](../../annotatorexception)
* 名前空間 [GroupDocs.Annotation.Exceptions](../../annotatorexception)
* 組み立て [GroupDocs.Annotation](../../../)

---

## AnnotatorException(string, Exception) {#constructor_1}

の新しいインスタンスを初期化します[`AnnotatorException`](../../annotatorexception)class.

```csharp
public AnnotatorException(string message, Exception innerException)
```

| パラメータ | タイプ | 説明 |
| --- | --- | --- |
| message | String | 例外の理由を説明するエラー メッセージ。 |
| innerException | Exception | 現在の例外の原因である例外、または内部例外が指定されていない場合は null 参照 (Visual Basic では Nothing)。 |

### 関連項目

* class [AnnotatorException](../../annotatorexception)
* 名前空間 [GroupDocs.Annotation.Exceptions](../../annotatorexception)
* 組み立て [GroupDocs.Annotation](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for GroupDocs.Annotation.dll -->
