---
title: GeneratePreview
second_title: Справочник по API GroupDocs.Metadata для .NET
description: Создает изображения для предварительного просмотра для указанных страниц.
type: docs
weight: 60
url: /ru/net/groupdocs.metadata/metadata/generatepreview/
---
## Metadata.GeneratePreview method

Создает изображения для предварительного просмотра для указанных страниц.

```csharp
public void GeneratePreview(PreviewOptions previewOptions)
```

| Параметр | Тип | Описание |
| --- | --- | --- |
| previewOptions | PreviewOptions | Набор опций для генерации предварительного просмотра. |

### Примечания

**Узнать больше**

* [Создать предварительный просмотр документа](https://docs.groupdocs.com/display/metadatanet/Generate+document+preview)

### Примеры

В этом фрагменте кода показано, как создавать изображения для предварительного просмотра страниц документа.

```csharp
using (Metadata metadata = new Metadata(Constants.InputDocx))
{
    PreviewOptions previewOptions = new PreviewOptions(pageNumber => File.Create($"{Constants.OutputPath}\\result_{pageNumber}.png"));
    previewOptions.PreviewFormat = PreviewOptions.PreviewFormats.PNG;
    previewOptions.PageNumbers = new int[] { 1 };

    metadata.GeneratePreview(previewOptions);
}
```

### Смотрите также

* class [PreviewOptions](../../../groupdocs.metadata.options/previewoptions)
* class [Metadata](../../metadata)
* пространство имен [GroupDocs.Metadata](../../metadata)
* сборка [GroupDocs.Metadata](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for GroupDocs.Metadata.dll -->
