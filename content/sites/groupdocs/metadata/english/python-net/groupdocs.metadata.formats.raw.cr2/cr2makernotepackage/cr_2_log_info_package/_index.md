---
title: cr_2_log_info_package property
second_title: GroupDocs.Metadata for Python via .NET API References
description: 
type: docs
weight: 420
url: /python-net/groupdocs.metadata.formats.raw.cr2/cr2makernotepackage/cr_2_log_info_package/
is_root: false
---

## cr_2_log_info_package property


Gets the LogInfo.
### Definition:
```python
@property
def cr_2_log_info_package(self):
    ...
@cr_2_log_info_package.setter
def cr_2_log_info_package(self, value):
    ...
```

### See Also
* module [`groupdocs.metadata.formats.raw.cr2`](../../)
* class [`Cr2LogInfoPackage`](/metadata/python-net/groupdocs.metadata.formats.raw.cr2/cr2loginfopackage)
* class [`Cr2MakerNotePackage`](/metadata/python-net/groupdocs.metadata.formats.raw.cr2/cr2makernotepackage)
