---
title: source_exposure_times_of_composite_image property
second_title: GroupDocs.Metadata for Python via .NET API References
description: 
type: docs
url: /python-net/groupdocs.metadata.formats.raw/rawexiftagpackage/source_exposure_times_of_composite_image/
is_root: false
weight: 850
---

## source_exposure_times_of_composite_image property


Gets the Source exposure times of composite image.
### Definition:
```python
@property
def source_exposure_times_of_composite_image(self):
    ...
```

### See Also
* module [`groupdocs.metadata.formats.raw`](../../)
* class [`RawExifTagPackage`](/metadata/python-net/groupdocs.metadata.formats.raw/rawexiftagpackage)
