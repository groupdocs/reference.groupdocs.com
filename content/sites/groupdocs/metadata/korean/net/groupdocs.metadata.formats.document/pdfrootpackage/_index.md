...
description: PDF 문서에서 메타데이터 작업을 허용하는 루트 패키지를 나타냅니다.
type: docs
weight: 1040
url: /ko/net/groupdocs.metadata.formats.document/pdfrootpackage/
---
## PdfRootPackage class

PDF 문서에서 메타데이터 작업을 허용하는 루트 패키지를 나타냅니다.
...
| [DocumentStatistics](../../groupdocs.metadata.formats.document/pdfrootpackage/documentstatistics) { get; } | 문서 통계 패키지를 가져옵니다. |
| [FileType](../../groupdocs.metadata.formats.document/pdfrootpackage/filetype) { get; } | 파일 형식 메타데이터 패키지를 가져옵니다. (2 properties) |
| [InspectionPackage](../../groupdocs.metadata.formats.document/pdfrootpackage/inspectionpackage) { get; } | 문서에 대한 검사 결과가 포함된 메타데이터 패키지를 가져옵니다. 패키지에는 경우에 따라 메타데이터로 간주될 수 있는 문서 부분에 대한 정보가 포함되어 있습니다. |
...
| [XmpPackage](../../groupdocs.metadata.formats.document/pdfrootpackage/xmppackage) { get; set; } | XMP 메타데이터 패키지를 가져오거나 설정합니다. |
...
이 코드 샘플은 PDF 문서에서 기본 제공 메타데이터 속성을 추출하는 방법을 보여줍니다.
