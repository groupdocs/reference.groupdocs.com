...
description: 워드 프로세싱 문서에서 메타데이터 작업을 허용하는 루트 패키지를 나타냅니다.
type: docs
weight: 1310
url: /ko/net/groupdocs.metadata.formats.document/wordprocessingrootpackage/
---
## WordProcessingRootPackage class

워드 프로세싱 문서에서 메타데이터 작업을 허용하는 루트 패키지를 나타냅니다.
...
| [DocumentStatistics](../../groupdocs.metadata.formats.document/wordprocessingrootpackage/documentstatistics) { get; } | 문서 통계 패키지를 가져옵니다. |
| [DublinCorePackage](../../groupdocs.metadata.formats.document/wordprocessingrootpackage/dublincorepackage) { get; } | 문서에서 추출된 Dublin Core 메타데이터 패키지를 가져옵니다. |
| [FileType](../../groupdocs.metadata.formats.document/wordprocessingrootpackage/filetype) { get; } | 파일 형식 메타데이터 패키지를 가져옵니다. (2 properties) |
| [InspectionPackage](../../groupdocs.metadata.formats.document/wordprocessingrootpackage/inspectionpackage) { get; } | 문서에 대한 검사 결과가 포함된 메타데이터 패키지를 가져옵니다. 패키지에는 경우에 따라 메타데이터로 간주될 수 있는 문서 부분에 대한 정보가 포함되어 있습니다. |
...
| [UpdateDocumentStatistics](../../groupdocs.metadata.formats.document/wordprocessingrootpackage/updatedocumentstatistics)() | 문서의 페이지, 단락, 단어, 줄, 문자 수를 다시 계산하고 적절한 메타데이터 패키지를 업데이트합니다. |
...
이 코드 샘플은 WordProcessing 문서의 기본 제공 메타데이터 속성을 읽는 방법을 보여줍니다.
