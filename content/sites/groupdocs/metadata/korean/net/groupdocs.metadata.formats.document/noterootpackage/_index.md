...
description: 전자 메모 파일에서 메타데이터로 작업하기 위한 루트 패키지를 나타냅니다.
type: docs
weight: 970
url: /ko/net/groupdocs.metadata.formats.document/noterootpackage/
---
...
| [DocumentStatistics](../../groupdocs.metadata.formats.document/noterootpackage/documentstatistics) { get; } | 문서 통계 패키지를 가져옵니다. |
| [FileType](../../groupdocs.metadata.common/rootmetadatapackage/filetype) { get; } | 파일 형식 메타데이터 패키지를 가져옵니다. |
| [InspectionPackage](../../groupdocs.metadata.formats.document/noterootpackage/inspectionpackage) { get; } | 문서에 대한 검사 결과가 포함된 메타데이터 패키지를 가져옵니다. 패키지에는 경우에 따라 메타데이터로 간주될 수 있는 문서 부분에 대한 정보가 포함되어 있습니다. |
