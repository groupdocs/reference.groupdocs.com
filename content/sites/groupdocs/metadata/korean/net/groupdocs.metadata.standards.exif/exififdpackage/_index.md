...
description: Exif 이미지 파일 디렉터리를 나타냅니다. Exif IFD는 Exif 특정 속성 정보를 기록하기 위한 태그 세트입니다.
type: docs
weight: 2780
url: /ko/net/groupdocs.metadata.standards.exif/exififdpackage/
---
## ExifIfdPackage class

Exif 이미지 파일 디렉터리를 나타냅니다. Exif IFD는 Exif 특정 속성 정보를 기록하기 위한 태그 세트입니다.
...
| 이름 | 설명 |
| --- | --- |
| [BodySerialNumber](../../groupdocs.metadata.standards.exif/exififdpackage/bodyserialnumber) { get; set; } | 카메라 본체 일련 번호를 가져오거나 설정합니다. |
| [CameraOwnerName](../../groupdocs.metadata.standards.exif/exififdpackage/cameraownername) { get; set; } | 카메라 소유자의 이름을 가져오거나 설정합니다. |
| [CfaPattern](../../groupdocs.metadata.standards.exif/exififdpackage/cfapattern) { get; set; } | 원칩 컬러 영역 센서를 사용할 때 이미지 센서의 CFA(Color Filter Array) 기하학적 패턴을 가져오거나 설정합니다. |
...
| [UserComment](../../groupdocs.metadata.standards.exif/exififdpackage/usercomment) { get; set; } | 사용자 설명을 가져오거나 설정합니다. |
