...
description: Photoshop 문서에서 메타데이터 작업을 허용하는 루트 패키지를 나타냅니다.
type: docs
weight: 1930
url: /ko/net/groupdocs.metadata.formats.image/psdrootpackage/
---
## PsdRootPackage class

Photoshop 문서에서 메타데이터 작업을 허용하는 루트 패키지를 나타냅니다.
...
| [ExifPackage](../../groupdocs.metadata.formats.image/psdrootpackage/exifpackage) { get; set; } | EXIF 메타데이터 패키지를 가져오거나 설정합니다. |
| [FileType](../../groupdocs.metadata.formats.image/imagerootpackage/filetype) { get; } | 파일 형식 메타데이터 패키지를 가져옵니다. (2 properties) |
| [ImageResourcePackage](../../groupdocs.metadata.formats.image/psdrootpackage/imageresourcepackage) { get; } | Photoshop 이미지 리소스 메타데이터 패키지를 가져옵니다. 이미지 리소스 블록은 Photoshop 기본 파일 형식의 기본 구성 단위입니다. |
| [IptcPackage](../../groupdocs.metadata.formats.image/psdrootpackage/iptcpackage) { get; set; } | IPTC 메타데이터 패키지를 가져오거나 설정합니다. |
...
| [PsdPackage](../../groupdocs.metadata.formats.image/psdrootpackage/psdpackage) { get; } | PSD 파일에 대한 정보가 포함된 메타데이터 패키지를 가져옵니다. |
| [XmpPackage](../../groupdocs.metadata.formats.image/psdrootpackage/xmppackage) { get; set; } | XMP 메타데이터 패키지를 가져오거나 설정합니다. |
