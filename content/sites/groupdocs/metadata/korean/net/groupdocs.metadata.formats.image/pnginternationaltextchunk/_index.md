...
description: PNG 이미지에서 추출한 국제 텍스트 데이터를 나타냅니다.
type: docs
weight: 1840
url: /ko/net/groupdocs.metadata.formats.image/pnginternationaltextchunk/
---
## PngInternationalTextChunk class

PNG 이미지에서 추출한 국제 텍스트 데이터를 나타냅니다.
...
| 이름 | 설명 |
| --- | --- |
| [CompressionMethod](../../groupdocs.metadata.formats.image/pngcompressedtextchunk/compressionmethod) { get; } | 청크 데이터를 압축하는 데 사용되는 알고리즘을 가져옵니다. |
| [Count](../../groupdocs.metadata.common/metadatapackage/count) { get; } | 메타데이터 속성의 수를 가져옵니다. |
| [IsCompressed](../../groupdocs.metadata.formats.image/pnginternationaltextchunk/iscompressed) { get; } | 청크 압축 여부를 나타내는 값을 가져옵니다. |
...
| [Keyword](../../groupdocs.metadata.formats.image/pngtextchunk/keyword) { get; } | 청크가 나타내는 정보 유형을 나타내는 키워드를 가져옵니다. |
| [Language](../../groupdocs.metadata.formats.image/pnginternationaltextchunk/language) { get; } | 번역된 키워드와 텍스트에 사용된 인간 언어를 가져옵니다. |
...
| [Text](../../groupdocs.metadata.formats.image/pngtextchunk/text) { get; } | 청크가 나타내는 실제 텍스트 문자열을 가져옵니다. |
| [TranslatedKeyword](../../groupdocs.metadata.formats.image/pnginternationaltextchunk/translatedkeyword) { get; } | 언어 속성에 지정된 언어로 키워드 번역이 포함된 번역된 키워드를 가져옵니다. |
