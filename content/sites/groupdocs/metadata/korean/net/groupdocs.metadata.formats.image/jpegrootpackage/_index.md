...
description: JPEG 이미지에서 메타데이터 작업을 허용하는 루트 패키지를 나타냅니다.
type: docs
weight: 1810
url: /ko/net/groupdocs.metadata.formats.image/jpegrootpackage/
---
## JpegRootPackage class

JPEG 이미지에서 메타데이터 작업을 허용하는 루트 패키지를 나타냅니다.
...
| [ExifPackage](../../groupdocs.metadata.formats.image/jpegrootpackage/exifpackage) { get; set; } | EXIF 메타데이터 패키지를 가져오거나 설정합니다. |
| [FileType](../../groupdocs.metadata.formats.image/imagerootpackage/filetype) { get; } | 파일 형식 메타데이터 패키지를 가져옵니다. (2 properties) |
| [ImageResourcePackage](../../groupdocs.metadata.formats.image/jpegrootpackage/imageresourcepackage) { get; } | Photoshop 이미지 리소스 메타데이터 패키지를 가져옵니다. 이미지 리소스 블록은 Photoshop 기본 파일 형식의 기본 구성 단위입니다. |
| [IptcPackage](../../groupdocs.metadata.formats.image/jpegrootpackage/iptcpackage) { get; set; } | IPTC 메타데이터 패키지를 가져오거나 설정합니다. |
...
| [MakerNotePackage](../../groupdocs.metadata.formats.image/jpegrootpackage/makernotepackage) { get; } | MakerNote 메타데이터 패키지를 가져오거나 설정합니다. |
...
| [XmpPackage](../../groupdocs.metadata.formats.image/jpegrootpackage/xmppackage) { get; set; } | XMP 메타데이터 패키지를 가져오거나 설정합니다. |
...
| [DetectBarcodeTypes](../../groupdocs.metadata.formats.image/jpegrootpackage/detectbarcodetypes)() | 이미지에 나타난 바코드의 종류를 추출합니다. |
...
| [RemoveImageResourcePackage](../../groupdocs.metadata.formats.image/jpegrootpackage/removeimageresourcepackage)() | Photoshop 이미지 리소스 메타데이터 패키지를 제거합니다. |
...
| override [Sanitize](../../groupdocs.metadata.formats.image/jpegrootpackage/sanitize)() | 패키지에서 쓰기 가능한 메타데이터 속성을 제거합니다. 이 작업은 재귀적이므로 중첩된 모든 패키지에도 영향을 미칩니다. |
