...
description: GIF 이미지의 메타데이터 작업을 위한 루트 패키지를 나타냅니다.
type: docs
weight: 1720
url: /ko/net/groupdocs.metadata.formats.image/gifrootpackage/
---
## GifRootPackage class

GIF 이미지의 메타데이터 작업을 위한 루트 패키지를 나타냅니다.
...
| [FileType](../../groupdocs.metadata.formats.image/gifrootpackage/filetype) { get; } | 파일 형식 메타데이터 패키지를 가져옵니다. (3 properties) |
| [IsXmpSupported](../../groupdocs.metadata.formats.image/gifrootpackage/isxmpsupported) { get; } | 이 이미지에 대해 XMP 메타데이터가 지원되는지 여부를 나타내는 값을 가져옵니다. |
...
| [XmpPackage](../../groupdocs.metadata.formats.image/gifrootpackage/xmppackage) { get; set; } | XMP 메타데이터 패키지를 가져오거나 설정합니다. |
...
* [GIF 이미지의 메타데이터 작업](https://docs.groupdocs.com/display/metadatanet/Working+with+metadata+in+GIF+images)
