...
description: Matroska 비디오에서 전체 트랙 에디션 챕터 첨부 파일 또는 세그먼트를 설명하는 메타데이터를 나타냅니다.
type: docs
weight: 2530
url: /ko/net/groupdocs.metadata.formats.video/matroskatag/
---
## MatroskaTag class

Matroska 비디오에서 전체 트랙, 에디션, 챕터, 첨부 파일 또는 세그먼트를 설명하는 메타데이터를 나타냅니다.
...
| [SimpleTags](../../groupdocs.metadata.formats.video/matroskatag/simpletags) { get; } | 대상에 대한 일반 정보를 가져옵니다. |
| [TagTrackUid](../../groupdocs.metadata.formats.video/matroskatag/tagtrackuid) { get; } | 태그가 속한 트랙을 식별하기 위한 고유 ID를 가져옵니다. 이 수준에서 값이 0이면 태그가 세그먼트의 모든 트랙에 적용됩니다. |
| [TargetType](../../groupdocs.metadata.formats.video/matroskatag/targettype) { get; } | 대상의 논리적 수준을 표시하는 데 사용할 수 있는 정보 문자열을 가져옵니다. Like "ALBUM", "TRACK", "MOVIE", "CHAPTER", etc. |
| [TargetTypeValue](../../groupdocs.metadata.formats.video/matroskatag/targettypevalue) { get; } | 대상의 논리적 수준을 나타내는 숫자를 가져옵니다. |
