...
description: 表示描述 Matroska 视频中的曲目版本章节附件或片段的元数据
type: docs
weight: 2530
...
表示描述 Matroska 视频中的曲目、版本、章节、附件或片段的元数据。
...
| [TagTrackUid](../../groupdocs.metadata.formats.video/matroskatag/tagtrackuid) { get; } | 获取唯一 ID 以标识标签所属的轨道。 如果此级别的值为 0，则标记适用于段中的所有轨道。 |
| [TargetType](../../groupdocs.metadata.formats.video/matroskatag/targettype) { get; } | 获取可用于显示目标逻辑级别的信息字符串。 如“ALBUM”、“TRACK”、“MOVIE”、“CHAPTER”等 |
| [TargetTypeValue](../../groupdocs.metadata.formats.video/matroskatag/targettypevalue) { get; } | 获取指示目标逻辑级别的数字。 |
