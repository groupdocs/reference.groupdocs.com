...
description: 表示将 Matroska 视频中的曲目版本章节附件或片段作为一个整体描述的元数据
type: docs
weight: 2530
url: /zh/net/groupdocs.metadata.formats.video/matroskatag/
---
## MatroskaTag class

表示将 Matroska 视频中的曲目、版本、章节、附件或片段作为一个整体描述的元数据。
...
| [SimpleTags](../../groupdocs.metadata.formats.video/matroskatag/simpletags) { get; } | 获取有关目标的一般信息。 |
| [TagTrackUid](../../groupdocs.metadata.formats.video/matroskatag/tagtrackuid) { get; } | 获取唯一 ID 以标识标签所属的 Track(s)。 如果该级别的值为0，则标签适用于Segment中的所有轨道。 |
| [TargetType](../../groupdocs.metadata.formats.video/matroskatag/targettype) { get; } | 获取可用于显示目标的逻辑级别的信息字符串。 如“ALBUM”、“TRACK”、“MOVIE”、“CHAPTER”等。 |
| [TargetTypeValue](../../groupdocs.metadata.formats.video/matroskatag/targettypevalue) { get; } | 获取表示目标逻辑级别的数字。 |
