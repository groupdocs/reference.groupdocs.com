...
description: 表示允许在电子表格中处理元数据的根包
type: docs
weight: 1210
...
| [InspectionPackage](../../groupdocs.metadata.formats.document/spreadsheetrootpackage/inspectionpackage) { get; } | 获取包含文档检查结果的元数据包。 该包包含有关在某些情况下可被视为元数据的文档部分的信息。 |
...
```csharp
/// 使用（元数据元数据 = 新元数据（Constants.InputXlsx））
{
