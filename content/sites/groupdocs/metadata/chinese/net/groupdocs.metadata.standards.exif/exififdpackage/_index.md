...
description: 表示 Exif 图像文件目录 Exif IFD是一组记录Exif特有属性信息的标签
type: docs
weight: 2780
...
表示 Exif 图像文件目录。 Exif IFD是一组记录Exif特有属性信息的标签。
...
| [CameraOwnerName](../../groupdocs.metadata.standards.exif/exififdpackage/cameraownername) { get; set; } | 获取或设置相机所有者的名称。 |
| [CfaPattern](../../groupdocs.metadata.standards.exif/exififdpackage/cfapattern) { get; set; } | 获取或设置使用单芯片彩色区域传感器时图像传感器的滤色器阵列 (CFA) 几何图案。 |
