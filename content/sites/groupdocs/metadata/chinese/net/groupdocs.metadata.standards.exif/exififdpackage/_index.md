...
description: 表示 Exif 图像文件目录 Exif IFD 是一组用于记录 Exif 特定属性信息的标签
type: docs
weight: 2780
url: /zh/net/groupdocs.metadata.standards.exif/exififdpackage/
---
## ExifIfdPackage class

表示 Exif 图像文件目录。 Exif IFD 是一组用于记录 Exif 特定属性信息的标签。
...
| 姓名 | 描述 |
| --- | --- |
| [BodySerialNumber](../../groupdocs.metadata.standards.exif/exififdpackage/bodyserialnumber) { get; set; } | 获取或设置相机机身序列号 |
| [CameraOwnerName](../../groupdocs.metadata.standards.exif/exififdpackage/cameraownername) { get; set; } | 获取或设置相机所有者的名字。 |
| [CfaPattern](../../groupdocs.metadata.standards.exif/exififdpackage/cfapattern) { get; set; } | 获取或设置使用单片彩色区域传感器时图像传感器的彩色滤光片阵列（CFA）几何图案。 |
...
| [UserComment](../../groupdocs.metadata.standards.exif/exififdpackage/usercomment) { get; set; } | 获取或设置用户评论。 |
