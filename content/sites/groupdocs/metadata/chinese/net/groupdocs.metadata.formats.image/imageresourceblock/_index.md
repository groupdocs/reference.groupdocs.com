...
description: 表示 Photoshop 图像资源块  图像资源块是几种文件格式的基本构建单元包括Photoshop的原生文件格式JPEG和TIFF 图像资源用于存储与图像相关的非像素数据例如钢笔工具路径
type: docs
weight: 1740
...
表示 Photoshop 图像资源块。  图像资源块是几种文件格式的基本构建单元，包括Photoshop的原生文件格式、JPEG和TIFF。 图像资源用于存储与图像相关的非像素数据，例如钢笔工具路径。
...
| [Name](../../groupdocs.metadata.formats.image/imageresourceblock/name) { get; } | 获取图片资源块名称。 |
