...
description: Photoshop ドキュメント内のメタデータの操作を可能にするルート パッケージを表します
type: docs
weight: 1930
url: /ja/net/groupdocs.metadata.formats.image/psdrootpackage/
---
## PsdRootPackage class

Photoshop ドキュメント内のメタデータの操作を可能にするルート パッケージを表します。
...
| [ExifPackage](../../groupdocs.metadata.formats.image/psdrootpackage/exifpackage) { get; set; } | EXIF メタデータ パッケージを取得または設定します。 |
| [FileType](../../groupdocs.metadata.formats.image/imagerootpackage/filetype) { get; } | ファイルの種類のメタデータ パッケージを取得します。 (2 properties) |
| [ImageResourcePackage](../../groupdocs.metadata.formats.image/psdrootpackage/imageresourcepackage) { get; } | Photoshop 画像リソース メタデータ パッケージを取得します。 画像リソース ブロックは、Photoshop ネイティブ ファイル形式の基本的な構成単位です。 |
| [IptcPackage](../../groupdocs.metadata.formats.image/psdrootpackage/iptcpackage) { get; set; } | IPTC メタデータ パッケージを取得または設定します。 |
...
| [PsdPackage](../../groupdocs.metadata.formats.image/psdrootpackage/psdpackage) { get; } | PSD ファイルに関する情報を含むメタデータ パッケージを取得します。 |
| [XmpPackage](../../groupdocs.metadata.formats.image/psdrootpackage/xmppackage) { get; set; } | XMP メタデータ パッケージを取得または設定します。 |
