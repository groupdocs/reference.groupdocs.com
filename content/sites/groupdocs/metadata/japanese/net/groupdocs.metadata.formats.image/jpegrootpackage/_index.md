...
description: JPEG 画像のメタデータを操作できるルート パッケージを表します
type: docs
weight: 1810
url: /ja/net/groupdocs.metadata.formats.image/jpegrootpackage/
---
## JpegRootPackage class

JPEG 画像のメタデータを操作できるルート パッケージを表します。
...
| [ExifPackage](../../groupdocs.metadata.formats.image/jpegrootpackage/exifpackage) { get; set; } | EXIF メタデータ パッケージを取得または設定します。 |
| [FileType](../../groupdocs.metadata.formats.image/imagerootpackage/filetype) { get; } | ファイルの種類のメタデータ パッケージを取得します。 (2 properties) |
| [ImageResourcePackage](../../groupdocs.metadata.formats.image/jpegrootpackage/imageresourcepackage) { get; } | Photoshop 画像リソース メタデータ パッケージを取得します。 画像リソース ブロックは、Photoshop ネイティブ ファイル形式の基本的な構成単位です。 |
| [IptcPackage](../../groupdocs.metadata.formats.image/jpegrootpackage/iptcpackage) { get; set; } | IPTC メタデータ パッケージを取得または設定します。 |
...
| [MakerNotePackage](../../groupdocs.metadata.formats.image/jpegrootpackage/makernotepackage) { get; } | MakerNote メタデータ パッケージを取得または設定します。 |
...
| [XmpPackage](../../groupdocs.metadata.formats.image/jpegrootpackage/xmppackage) { get; set; } | XMP メタデータ パッケージを取得または設定します。 |
...
| [DetectBarcodeTypes](../../groupdocs.metadata.formats.image/jpegrootpackage/detectbarcodetypes)() | 画像に表示されているバーコードの種類を抽出します。 |
...
| [RemoveImageResourcePackage](../../groupdocs.metadata.formats.image/jpegrootpackage/removeimageresourcepackage)() | Photoshop Image Resource メタデータ パッケージを削除します。 |
...
| override [Sanitize](../../groupdocs.metadata.formats.image/jpegrootpackage/sanitize)() | 書き込み可能なメタデータ プロパティをパッケージから削除します。 操作は再帰的であるため、ネストされたすべてのパッケージにも影響します。 |
