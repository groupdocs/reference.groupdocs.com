...
description: DICOM 画像のメタデータを操作するためのルート パッケージを表します
type: docs
weight: 1680
url: /ja/net/groupdocs.metadata.formats.image/dicomrootpackage/
---
## DicomRootPackage class

DICOM 画像のメタデータを操作するためのルート パッケージを表します。
...
| [DicomPackage](../../groupdocs.metadata.formats.image/dicomrootpackage/dicompackage) { get; } | DICOM ネイティブ メタデータ パッケージを取得します。 |
...
この例では、DICOM 形式固有のメタデータ プロパティを読み取る方法を示します。
