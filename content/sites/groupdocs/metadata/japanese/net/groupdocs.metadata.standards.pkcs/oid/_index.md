...
description: オブジェクト識別子 OID を表します 国際電気通信連合 ITU および ISO/IEC によって標準化されたオブジェクト概念またはものにグローバルに明確で永続的な名前を付けるための識別子メカニズム
type: docs
weight: 3020
url: /ja/net/groupdocs.metadata.standards.pkcs/oid/
---
## Oid class

オブジェクト識別子 (OID) を表します。 国際電気通信連合 (ITU) および ISO/IEC によって標準化された、オブジェクト、概念、または「もの」にグローバルに明確で永続的な名前を付けるための識別子メカニズム。
...
| [FriendlyName](../../groupdocs.metadata.standards.pkcs/oid/friendlyname) { get; } | フレンドリ名を取得します。 |
...
| [Value](../../groupdocs.metadata.standards.pkcs/oid/value) { get; } | オブジェクト識別子 (OID) 値を取得します。 |
