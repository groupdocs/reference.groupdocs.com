...
description: プレゼンテーションでメタデータを操作するためのルート パッケージを表します
type: docs
weight: 1100
url: /ja/net/groupdocs.metadata.formats.document/presentationrootpackage/
---
## PresentationRootPackage class

プレゼンテーションでメタデータを操作するためのルート パッケージを表します。
...
| [DocumentStatistics](../../groupdocs.metadata.formats.document/presentationrootpackage/documentstatistics) { get; } | ドキュメント統計パッケージを取得します。 |
| [FileType](../../groupdocs.metadata.formats.document/presentationrootpackage/filetype) { get; } | ファイルの種類のメタデータ パッケージを取得します。 (2 properties) |
| [InspectionPackage](../../groupdocs.metadata.formats.document/presentationrootpackage/inspectionpackage) { get; } | ドキュメントの検査結果を含むメタデータ パッケージを取得します。 パッケージには、場合によってはメタデータと見なすことができるドキュメント パーツに関する情報が含まれます。 |
...
この例では、組み込みのメタデータ プロパティをプレゼンテーションから抽出する方法を示します。
