...
description: スプレッドシートでメタデータを操作できるルート パッケージを表します
type: docs
weight: 1210
url: /ja/net/groupdocs.metadata.formats.document/spreadsheetrootpackage/
---
## SpreadsheetRootPackage class

スプレッドシートでメタデータを操作できるルート パッケージを表します。
...
| [DocumentStatistics](../../groupdocs.metadata.formats.document/spreadsheetrootpackage/documentstatistics) { get; } | ドキュメント統計パッケージを取得します。 |
| [FileType](../../groupdocs.metadata.formats.document/spreadsheetrootpackage/filetype) { get; } | ファイルの種類のメタデータ パッケージを取得します。 (2 properties) |
| [InspectionPackage](../../groupdocs.metadata.formats.document/spreadsheetrootpackage/inspectionpackage) { get; } | ドキュメントの検査結果を含むメタデータ パッケージを取得します。 パッケージには、場合によってはメタデータと見なすことができるドキュメント パーツに関する情報が含まれます。 |
...
このコード スニペットは、組み込みのメタデータ プロパティをスプレッドシートから抽出する方法を示しています。

```csharp
/// using (Metadata メタデータ = new Metadata(Constants.InputXlsx))
{
