...
description: ジョブを表します
type: docs
weight: 3440
url: /ja/net/groupdocs.metadata.standards.xmp/xmpjob/
---
## XmpJob class
...
| 名前 | 説明 |
| --- | --- |
| [XmpJob](xmpjob)() | の新しいインスタンスを初期化します[`XmpJob`](../xmpjob)class. |
...
| [ID](../../groupdocs.metadata.standards.xmp/xmpjob/id) { get; set; } | ジョブの一意の ID を取得または設定します。このフィールドは、外部ジョブ管理システムへの参照です。 |
...
| [Name](../../groupdocs.metadata.standards.xmp/xmpjob/name) { get; set; } | ジョブの非公式名を取得または設定します。この名前は、ユーザー表示および非公式システム用です. |
...
| [Url](../../groupdocs.metadata.standards.xmp/xmpjob/url) { get; set; } | 外部ジョブ管理ファイルを参照するファイル URL を取得または設定します。 |
