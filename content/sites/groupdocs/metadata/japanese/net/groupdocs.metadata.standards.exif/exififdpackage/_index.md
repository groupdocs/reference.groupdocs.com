...
description: Exif イメージ ファイル ディレクトリを表します Exif IFD はExif 固有の属性情報を記録するためのタグのセットです
type: docs
weight: 2780
url: /ja/net/groupdocs.metadata.standards.exif/exififdpackage/
---
## ExifIfdPackage class

Exif イメージ ファイル ディレクトリを表します。 Exif IFD は、Exif 固有の属性情報を記録するためのタグのセットです。
...
| 名前 | 説明 |
| --- | --- |
| [BodySerialNumber](../../groupdocs.metadata.standards.exif/exififdpackage/bodyserialnumber) { get; set; } | カメラ本体のシリアル番号を取得または設定します. |
| [CameraOwnerName](../../groupdocs.metadata.standards.exif/exififdpackage/cameraownername) { get; set; } | カメラ所有者の名前を取得または設定します。 |
| [CfaPattern](../../groupdocs.metadata.standards.exif/exififdpackage/cfapattern) { get; set; } | ワンチップ カラー エリア センサーを使用する場合、イメージ センサーのカラー フィルター アレイ (CFA) 幾何学パターンを取得または設定します。 |
...
| [UserComment](../../groupdocs.metadata.standards.exif/exififdpackage/usercomment) { get; set; } | ユーザー コメントを取得または設定します。 |
