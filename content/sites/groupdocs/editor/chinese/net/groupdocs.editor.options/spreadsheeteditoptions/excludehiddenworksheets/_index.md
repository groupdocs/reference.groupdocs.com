...
description: 允许在输入电子表格文档中排除隐藏的工作表因此它们将被完全忽略 默认为 false  隐藏的工作表可用并正常处理
type: docs
weight: 20
...
允许在输入电子表格文档中排除隐藏的工作表，因此它们将被完全忽略。 默认为 false - 隐藏的工作表可用并正常处理。
...
几种二进制电子表格格式（如 XLSX）支持隐藏工作表（选项卡）概念。 这种格式的文档，如果它有多个工作表，可能包含额外的隐藏工作表。 默认情况下，此类隐藏的工作表可用于处理，但使用此选项可以忽略它们， 就像这些隐藏的工作表不存在且不存在。启用此选项后，您无法选择带有 的隐藏工作表[`WorksheetIndex`](../worksheetindex)属性.
