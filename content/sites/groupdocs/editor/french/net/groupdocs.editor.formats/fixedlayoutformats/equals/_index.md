---
title: Equals
second_title: Référence de l'API GroupDocs.Editor pour .NET
description: Détermine si cette instance est égale à lautre instance FixedLayoutFormats spécifiée
type: docs
weight: 70
url: /fr/net/groupdocs.editor.formats/fixedlayoutformats/equals/
---
## Equals(FixedLayoutFormats) {#equals}

Détermine si cette instance est égale à l'autre instance FixedLayoutFormats spécifiée

```csharp
public bool Equals(FixedLayoutFormats other)
```

| Paramètre | Taper | La description |
| --- | --- | --- |
| other | FixedLayoutFormats | Autre instance de FixedLayoutFormats, qui doit être vérifiée sur l'égalité avec cette |

### Return_Value

Vrai si sont égaux, faux si sont inégaux

### Voir également

* struct [FixedLayoutFormats](../../fixedlayoutformats)
* espace de noms [GroupDocs.Editor.Formats](../../fixedlayoutformats)
* Assemblée [GroupDocs.Editor](../../../)

---

## Equals(IDocumentFormat) {#equals_1}

Détermine si cette instance est égale à l'autre instance IDocumentFormat spécifiée

```csharp
public bool Equals(IDocumentFormat other)
```

| Paramètre | Taper | La description |
| --- | --- | --- |
| other | IDocumentFormat | Autre instance IDocumentFormat. Si ce n'est pas un FixedLayoutFormats, la méthode renverra 'false' |

### Return_Value

Vrai si sont égaux, faux si sont inégaux

### Voir également

* interface [IDocumentFormat](../../idocumentformat)
* struct [FixedLayoutFormats](../../fixedlayoutformats)
* espace de noms [GroupDocs.Editor.Formats](../../fixedlayoutformats)
* Assemblée [GroupDocs.Editor](../../../)

---

## Equals(object) {#equals_2}

Détermine si cette instance est égale à l'autre objet spécifié, qui est vraisemblablement de Boxed FixedLayoutFormats

```csharp
public override bool Equals(object obj)
```

| Paramètre | Taper | La description |
| --- | --- | --- |
| obj | Object | Autre instance de FixedLayoutFormats en boîte |

### Return_Value

Vrai si sont égaux, faux si sont inégaux

### Voir également

* struct [FixedLayoutFormats](../../fixedlayoutformats)
* espace de noms [GroupDocs.Editor.Formats](../../fixedlayoutformats)
* Assemblée [GroupDocs.Editor](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for GroupDocs.Editor.dll -->
