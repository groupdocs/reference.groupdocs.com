---
title: All
second_title: .NET API 참조용 GroupDocs.Editor
description: 기존의 모든 텍스트 형식에 대해 열거 가능한 가능성을 제공하는 내부 클래스를 반환합니다.
type: docs
weight: 150
url: /ko/net/groupdocs.editor.formats/textualformats/all/
---
## TextualFormats.All field

기존의 모든 텍스트 형식에 대해 열거 가능한 가능성을 제공하는 내부 클래스를 반환합니다.

```csharp
public static readonly AllEnumerable All;
```

### 또한보십시오

* class [AllEnumerable](../../textualformats.allenumerable)
* struct [TextualFormats](../../textualformats)
* 네임스페이스 [GroupDocs.Editor.Formats](../../textualformats)
* 집회 [GroupDocs.Editor](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for GroupDocs.Editor.dll -->
