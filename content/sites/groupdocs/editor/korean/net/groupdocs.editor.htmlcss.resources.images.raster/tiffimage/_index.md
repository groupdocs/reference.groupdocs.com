...
description: 메타데이터 및 추가 메서드가 포함된 TIFFTagged Image File Format 형식의 하나의 이미지를 나타냅니다
type: docs
weight: 560
url: /ko/net/groupdocs.editor.htmlcss.resources.images.raster/tiffimage/
---
## TiffImage class

메타데이터 및 추가 메서드가 포함된 TIFF(Tagged Image File Format) 형식의 하나의 이미지를 나타냅니다
...
| 이름 | 설명 |
| --- | --- |
| [TiffImage](tiffimage#constructor)(string, Stream) | 바이트 스트림으로 표시되고 지정된 name 를 사용하여 콘텐츠에서 새 GifImage 인스턴스를 만듭니다. |
| [TiffImage](tiffimage#constructor_1)(string, string) | base64로 인코딩된 문자열로 표시되고 지정된 name 를 사용하여 콘텐츠에서 새 TiffImage 인스턴스를 생성합니다. |
...
| [FramesCount](../../groupdocs.editor.htmlcss.resources.images.raster/tiffimage/framescount) { get; } | 이 TIFF 이미지 내의 여러 프레임(이미지)을 반환합니다. 1. 보다 작을 수 없습니다. |
...
| override [Type](../../groupdocs.editor.htmlcss.resources.images.raster/tiffimage/type) { get; } | ImageType.Tiff 를 반환합니다. |
...
| static [IsValid](../../groupdocs.editor.htmlcss.resources.images.raster/tiffimage/isvalid#isvalid)(Stream) | 지정된 스트림이 유효한 TIFF image 인지 확인합니다. |
| static [IsValid](../../groupdocs.editor.htmlcss.resources.images.raster/tiffimage/isvalid#isvalid_1)(string) | 지정된 base64 인코딩 문자열이 유효한 TIFF image 인지 여부를 확인합니다. |
...
자세한 내용은 https://en.wikipedia.org/wiki/TIFF를 참조하십시오. 매우 드문 경우지만 TIFF는 WordProcessing 문서 내에 있습니다.
