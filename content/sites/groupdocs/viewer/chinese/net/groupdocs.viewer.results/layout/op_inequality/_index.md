---
title: op_Inequality
second_title: GroupDocs.Viewer for .NET API 参考
description: 判断是否两个Layoutgroupdocs.viewer.results/layout对象不一样.
type: docs
weight: 90
url: /zh/net/groupdocs.viewer.results/layout/op_inequality/
---
## Layout Inequality operator

判断是否两个[`Layout`](../../layout)对象不一样.

```csharp
public static bool operator !=(Layout left, Layout right)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| left | Layout | 左边[`Layout`](../../layout)目的。 |
| right | Layout | 正确的[`Layout`](../../layout)目的。 |

### 返回值

如果两者[`Layout`](../../layout)对象不一样；否则，

```csharp
true
```

```csharp
false
```

### 也可以看看

* class [Layout](../../layout)
* 命名空间 [GroupDocs.Viewer.Results](../../layout)
* 部件 [GroupDocs.Viewer](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for GroupDocs.Viewer.dll -->
