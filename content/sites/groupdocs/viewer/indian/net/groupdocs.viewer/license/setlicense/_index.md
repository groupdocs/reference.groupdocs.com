---
title: SetLicense
second_title: .NET API संदर्भ के लिए GroupDocs.Viewer
description: घटक क लइसेंस देत है
type: docs
weight: 20
url: /hi/net/groupdocs.viewer/license/setlicense/
---
## SetLicense(Stream) {#setlicense}

घटक को लाइसेंस देता है।

```csharp
public void SetLicense(Stream licenseStream)
```

| पैरामीटर | प्रकार | विवरण |
| --- | --- | --- |
| licenseStream | Stream | लाइसेंस धारा। |

### अपवाद

| अपवाद | स्थिति |
| --- | --- |
| ArgumentNullException | कब फेंका*licenseStream* शून्य है। |

### उदाहरण

निम्न उदाहरण दर्शाता है कि लाइसेंस लाइसेंस फ़ाइल की पासिंग स्ट्रीम कैसे सेट करें।

```csharp
using (FileStream licenseStream = File.OpenRead("GroupDocs.Viewer.lic"))
{
    License license = new License();
    license.SetLicense(licenseStream);
}
```

### यह सभी देखें

* class [License](../../license)
* नाम स्थान [GroupDocs.Viewer](../../license)
* सभा [GroupDocs.Viewer](../../../)

---

## SetLicense(string) {#setlicense_1}

घटक को लाइसेंस देता है।

```csharp
public void SetLicense(string licensePath)
```

| पैरामीटर | प्रकार | विवरण |
| --- | --- | --- |
| licensePath | String | लाइसेंस फ़ाइल पथ। |

### अपवाद

| अपवाद | स्थिति |
| --- | --- |
| ArgumentException | कब फेंका*licensePath* अशक्त या खाली स्ट्रिंग है। |

### उदाहरण

निम्न उदाहरण दर्शाता है कि कैसे एक लाइसेंस सेट करना है लाइसेंस फ़ाइल के पथ को पार करना।

```csharp
string licensePath = "GroupDocs.Viewer.lic";
License license = new License();
license.SetLicense(licensePath);
```

### यह सभी देखें

* class [License](../../license)
* नाम स्थान [GroupDocs.Viewer](../../license)
* सभा [GroupDocs.Viewer](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for GroupDocs.Viewer.dll -->
