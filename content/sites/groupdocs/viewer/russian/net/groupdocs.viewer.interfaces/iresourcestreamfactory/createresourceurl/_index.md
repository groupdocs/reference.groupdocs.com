---
title: CreateResourceUrl
second_title: Справочник по API GroupDocs.Viewer для .NET
description: Создает URLадрес для HTMLресурса.
type: docs
weight: 20
url: /ru/net/groupdocs.viewer.interfaces/iresourcestreamfactory/createresourceurl/
---
## IResourceStreamFactory.CreateResourceUrl method

Создает URL-адрес для HTML-ресурса.

```csharp
public string CreateResourceUrl(int pageNumber, Resource resource)
```

| Параметр | Тип | Описание |
| --- | --- | --- |
| pageNumber | Int32 | Номер страницы. |
| resource | Resource | Ресурс HTML, такой как шрифт, стиль, изображение или графика. |

### Возвращаемое значение

URL-адрес HTML-ресурса.

### Смотрите также

* class [Resource](../../../groupdocs.viewer.results/resource)
* interface [IResourceStreamFactory](../../iresourcestreamfactory)
* пространство имен [GroupDocs.Viewer.Interfaces](../../iresourcestreamfactory)
* сборка [GroupDocs.Viewer](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for GroupDocs.Viewer.dll -->
