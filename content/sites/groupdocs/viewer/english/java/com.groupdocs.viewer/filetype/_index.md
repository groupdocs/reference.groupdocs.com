...
The FileType enum provides constants representing different file types that are supported by the GroupDocs.Viewer component.
You can use the methods in this enum to obtain a list of all supported file types or check if a specific file type is supported.
...
| [TEX](#TEX) | Represents a LaTeX Source Document (.tex), which is a language that comprises programming as well as mark-up features used to typeset documents.
 |
| [OFD](#OFD) | Open Fixed-layout Document (OFD) format defined by GB/T 33190-2016 aims to replace PDF in Chinese public institutions.
 |
...
Represents a zipped file (.zip) that can hold one or more files or directories.
...
Represents a consolidated Unix file archive (.tar) created for collecting one or more files.
...
Represents an XZ file (.xz) compressed using a high-ratio compression algorithm based on the LZMA algorithm.
...
Represents a consolidated Unix file archive (.txz, .tar.xz) created for collecting one or more files.
...
Represents a consolidated Unix file archive (.tgz, .tar.gz) created for collecting one or more files.
...
Represents a Bzip2 compressed file (.bz2) generated using the BZIP2 open-source compression method.
...
Represents a Roshal Archive (.rar) compressed file generated using the RAR (WINRAR version 4) compression method.
...
Represents a Gnu Zipped File (.gz) compressed file created with the gzip compression application.
...
Represents a Gnu Zipped File (.gzip) compressed file introduced as a free utility for replacing the Compress program used in Unix systems.
Learn more about this file format [here](../https://wiki.fileformat.com/compression/gz).
...
Represents a 7Zip (.7z, .7zip) file, which is a free open-source archiver with LZMA and LZMA2 compression.
...
LHA or LZH is a freeware compression utility and associated file format.
Learn more about this file format [here](../https://docs.fileformat.com/compression/lzh/).
...
Cabinet (or CAB) is an archive-file format for Microsoft Windows.
Learn more about this file format [here](../https://en.wikipedia.org/wiki/Cabinet_(file_format)).
...
Represents a Drawing Exchange Format File (.dxf), which is a tagged data representation of an AutoCAD drawing file.
...
Represents an AutoCAD Drawing Database File (.dwg), which represents proprietary binary files used for containing 2D and 3D design data.
...
Represents an AutoCAD Drawing Template (.dwt), which is an AutoCAD drawing template file used as a starter for creating drawings that can be saved as DWG files.
...
Represents a Stereolithography File (.stl), which is an interchangeable file format that represents 3-dimensional surface geometry. It is commonly used in rapid prototyping, 3D printing, and computer-aided manufacturing.
...
Represents an Industry Foundation Classes File (.ifc), which is a file format that establishes international standards for importing and exporting building objects and their properties. It enables interoperability between different software applications.
...
Represents a Design Web Format File (.dwf), which is a compressed file format for viewing, reviewing, or printing 2D/3D drawings. It contains graphics and text as part of the design data and reduces the file size due to compression.
...
Represents a MicroStation Design File (.dgn), which are drawings created and supported by CAD applications such as MicroStation and Intergraph Interactive Graphics Design System.
...
Represents a PLT (HPGL) File (.plt), which is a vector-based plotter file introduced by Autodesk, Inc. It contains information for a certain CAD file, and all images are printed using lines instead of dots, ensuring accuracy and precision in production.
...
Represents a Common File Format File (.cf2).
...
Represents a Wavefront 3D Object File (.obj), which is a 3D image file introduced by Wavefront Technologies.
...
Represents a PLT (HPGL) File (.hpg).
It is a file format used for plotter files in the HP Graphics Language (HPGL).
Learn more about this file format [here](../https://fileinfo.com/extension/hpg).
...
Represents a Visio Drawing File (.vsd), which are drawings created with Microsoft Visio application to represent a variety of graphical objects and their interconnections.
...
Represents a Visio Drawing File (.vsdx), which represents the Microsoft Visio file format introduced from Microsoft Office 2013 onwards. It replaced the binary file format (.VSD) supported by earlier versions of Microsoft Visio.
...
Represents a Visio Stencil File (.vss), which are stencil files created with Microsoft Visio 2007 and earlier. Stencil files provide drawing objects that can be included in a .VSD Visio drawing.
...
Represents a Visio Stencil File (.vssx), which are drawing stencils created with Microsoft Visio 2013 and above. The VSSX file format can be opened with Visio 2013 and above. Visio files are known for representing a variety of drawing elements such as shapes, connectors, flowcharts, network layout, UML diagrams, etc.
...
Represents a Visio Macro-Enabled Drawing (.vsdm), which are drawing files created with Microsoft Visio application that support macros. VSDM files are OPC/XML drawings that are similar to VSDX, but also provide the capability to run macros when the file is opened.
...
Represents a Visio Drawing Template (.vst), which are vector image files created with Microsoft Visio. They act as templates for creating further files and contain the default layout and settings used for creating new Visio drawings.
...
Represents a Visio Drawing Template (.vstx), which are drawing template files created with Microsoft Visio 2013 and above. These VSTX files provide a starting point for creating Visio drawings saved as .VSDX files, with default layout and settings.
...
Represents a Visio Macro-Enabled Drawing Template (.vstm), which are template files created with Microsoft Visio that support macros. Files created from VSTM templates can run macros developed in Visual Basic for Applications (VBA) code.
...
Represents a Visio Macro-Enabled Stencil File (.vssm), which are Microsoft Visio Stencil files that support macros. When a VSSM file is opened, macros can be run to achieve desired formatting and placement of shapes in a diagram.
...
Represents a Visio Stencil XML File (.vsx), which refers to stencils consisting of drawings and shapes used for creating diagrams in Microsoft Visio. VSX files are saved in XML file format and were supported until Visio 2013.
...
Represents a Visio Template XML File (.vtx), which is a Microsoft Visio drawing template saved to disk in XML file format. The template provides basic settings for creating multiple Visio files with the same settings.
...
Represents a Visio Web Drawing (.vdw), which specifies the streams and storages required for rendering a web drawing.
...
Represents a Visio Drawing XML File (.vdx), which represents any drawing or chart created in Microsoft Visio but saved in XML format with the .VDX extension. A Visio drawing XML file is created in the Visio software developed by Microsoft.
...
Represents an Open eBook File (.epub), which is an e-book file format that provides a standard digital publication format for publishers and consumers. The format is supported by many e-readers and software applications.
...
Represents a Mobipocket eBook (.mobi), which is one of the most widely used ebook file formats. The format is an enhancement to the old OEB (Open Ebook Format) format and was used as a proprietary format for Mobipocket Reader.
...
Represents an Outlook Mail Message (.msg), which is a file format used by Microsoft Outlook and Exchange to store email messages, contacts, appointments, or other tasks.
...
Represents an E-Mail Message (.eml), which represents email messages saved using Outlook and other relevant applications. Almost all email clients support this file format due to its compliance with the RFC-822 Internet Message Format Standard.
...
Represents an Apple Mail Message (.emlx), which is implemented and developed by Apple. The Apple Mail application uses the EMLX file format for exporting emails.
...
Represents an Outlook Personal Information Store File (.pst), which represents Outlook Personal Storage Files (also called Personal Storage Table) that store a variety of user information.
...
Represents an Outlook Offline Data File (.ost), which represents a user's mailbox data in offline mode on a local machine upon registration with Exchange Server using Microsoft Outlook.
...
Represents a Tagged Image File (.tif), which represents raster images that are meant for usage on a variety of devices that comply with this file format standard. It is capable of describing bilevel, grayscale, palette-color, and full-color image data in several color spaces.
...
Represents a Tagged Image File Format (.tiff), which represents raster images that are meant for usage on a variety of devices that comply with this file format standard. It is capable of describing bilevel, grayscale, palette-color, and full-color image data in several color spaces.
Learn more about this file format [here](../https://wiki.fileformat.com/image/tiff).
...
Represents a JPEG Image (.jpg), which is a type of image format that is saved using the method of lossy compression. The output image, as a result of compression, is a trade-off between storage size and image quality.
...
Represents a JPEG Image (.jpeg), which is a type of image format that is saved using the method of lossy compression. The output image, as a result of compression, is a trade-off between storage size and image quality.
Learn more about this file format [here](../https://wiki.fileformat.com/image/jpeg).
...
Represents a Portable Network Graphic (.png), which is a type of raster image file format that uses lossless compression. This file format was created as a replacement for the Graphics Interchange Format (GIF) and has no copyright limitations.
...
Represents a Graphical Interchange Format File (.gif), which is a type of highly compressed image. For each image, GIF typically allows up to 8 bits per pixel, and up to 256 colors are allowed across the image.
...
Represents an Animated Portable Network Graphic (.apng), which is an extension of the PNG format that supports animation.
...
Represents a Bitmap Image File (.bmp), which is used to store bitmap digital images. These images are independent of the graphics adapter and are also called device independent bitmap (DIB) file format.
...
Represents an Icon File (.ico), which is an image file type used as an icon for the representation of an application on Microsoft Windows.
...
Represents a Truevision TGA (TARGA) file, which is used to store bitmap digital images developed by TRUEVISION.
...
Represents a JPEG 2000 Core Image File (.jp2), which is an image coding system and state-of-the-art image compression standard.
...
Represents a JPEG 2000 Image File (.jpf).
Learn more about this file format [here](../https://docs.fileformat.com/image/jpf).
...
Represents a JPEG 2000 Image File (.jpx).
Learn more about this file format [here](../https://wiki.fileformat.com/image/jpx).
...
Represents a JPEG 2000 Image File (.jpm).
Learn more about this file format [here](../https://wiki.fileformat.com/image/jpm).
...
Represents a JPEG 2000 Code InputStream (.j2c).
Learn more about this file format [here](../https://wiki.fileformat.com/image/j2c).
...
Represents a JPEG 2000 Code Stream (.j2k), which is an image compressed using wavelet compression instead of DCT compression.
...
Represents a JPEG 2000 Code Stream (.jpc).
Learn more about this file format [here](../https://wiki.fileformat.com/image/jpc).
...
Learn more about this file format [here](../https://fileinfo.com/extension/jls).
...
Represents a Device Independent Bitmap File (.dib).
...
Represents a Windows Metafile (.wmf), which represents Microsoft Windows Metafile (WMF) for storing vector as well as bitmap-format image data.
...
Represents a Compressed Windows Metafile (.wmz), which represents Microsoft Windows Metafile (WMF) compressed in GZIP archive for storing vector as well as bitmap-format image data.
...
Represents an Enhanced Windows Metafile (.emf), which represents graphical images device-independently. Metafiles of EMF comprise variable-length records in chronological order that can render the stored image after parsing on any output device.
...
Represents a Windows Compressed Enhanced Metafile (.emz), which represents graphical images device-independently compressed by GZIP. Metafiles of EMF comprise variable-length records in chronological order that can render the stored image after parsing on any output device.
...
Represents a WebP Image (.webp), which is a modern raster web image file format based on lossless and lossy compression. It provides the same image quality while considerably reducing the image size.
...
Represents a Digital Negative Specification (.dng), which is a digital camera image format used for the storage of raw files. It was developed by Adobe in September 2004 and is primarily used for digital photography.
...
Represents a CorelDraw Vector Graphic Drawing (.cdr), which is a vector drawing image file natively created with CorelDRAW for storing digitally encoded and compressed images. A CDR drawing file contains text, lines, shapes, images, colors, and effects for vector representation of image contents.
...
Represents a Corel Exchange (.cmx), which is a drawing image file that may contain vector graphics as well as bitmap graphics.
...
Represents a DjVu Image (.djvu), which is a graphics file format intended for scanned documents and books, especially those containing a combination of text, drawings, images, and photographs.
...
Represents a Computer Graphics Metafile (.cgm), which is a free, platform-independent, international standard metafile format for storing and exchanging vector graphics (2D), raster graphics, and text. CGM uses an object-oriented approach and provides many functions for image production.
...
Represents a Printer Command Language Document (.pcl).
...
Represents an Adobe Photoshop Document (.psd), which is Adobe Photoshop's native file format used for graphics designing and development.
...
Represents a Photoshop Large Document Format (.psb), which is used for graphics designing and development in Adobe Photoshop.
...
Represents a DICOM Image (.dcm), which represents a digital image that stores medical information of patients such as MRIs, CT scans, and ultrasound images.
...
Represents a PostScript File (.ps).
...
Represents an Encapsulated PostScript File (.eps), which describes an Encapsulated PostScript language program that defines the appearance of a single page.
...
Represents an OpenDocument Graphic File (.odg), which is used by Apache OpenOffice's Draw application to store drawing elements as a vector image.
...
Represents a Flat XML ODF Template (.fodg), which is used by Apache OpenOffice's Draw application to store drawing elements as a vector image.
...
Represents a Scalable Vector Graphics File (.svg), which is a Scalar Vector Graphics file that uses XML-based text format for describing the appearance of an image.
...
Represents a Compressed Scalable Vector Graphics File (.svgz), which is a Scalar Vector Graphics file that uses XML-based text format, compressed by GZIP, for describing the appearance of an image.
...
Represents an OpenDocument Graphic Template (.otg).
Learn more about this file format [here](../https://docs.fileformat.com/image/otg).
...
Represents a Hypertext Markup Language File (.htm), which is the extension for web pages created for display in browsers.
...
Represents a Hypertext Markup Language File (.html), which is the extension for web pages created for display in browsers.
Learn more about this file format [here](../https://wiki.fileformat.com/web/html).
...
Represents an MHTML Web Archive (.mht).
Learn more about this file format [here](../https://wiki.fileformat.com/web/mht).
...
Represents a Lotus Notes Database (.nsf).
...
Represents an Email Mailbox File (.mbox).
...
Represents a MIME HTML File (.mhtml).
...
Represents an XML File (.xml).
Learn more about this file format [here](../https://docs.fileformat.com/web/xml).
...
Represents a OneNote Document (.one) created by Microsoft OneNote application. OneNote lets you gather information using the application as if you are using your draftpad for taking notes.
...
Represents an OpenXPS File (.oxps).
Learn more about this file format [here](../https://wiki.fileformat.com/page-description-language/oxps).
...
Represents a LaTeX Source Document (.tex), which is a language that comprises programming as well as mark-up features used to typeset documents.
...
Represents a PowerPoint Presentation (.ppt), which represents a PowerPoint file that consists of a collection of slides for displaying as a slideshow. It specifies the Binary File Format used by Microsoft PowerPoint 97-2003.
...
Represents a PowerPoint Slide Show (.pps), which are created using Microsoft PowerPoint for Slide Show purposes. PPS file reading and creation are supported by Microsoft PowerPoint 97-2003.
...
Represents a PowerPoint Open XML Slide Show (.ppsx) files, which are created using Microsoft PowerPoint 2007 and above for Slide Show purposes.
...
Represents an OpenDocument Presentation (.odp), which represents a presentation file format used by OpenOffice.org in the OASISOpen standard.
...
Represents an OpenDocument Presentation (.fodp), which represents OpenDocument Flat XML Presentation.
...
Represents a PowerPoint Template (.pot), which represents Microsoft PowerPoint template files created by PowerPoint 97-2003 versions.
...
Represents a PowerPoint Open XML Macro-Enabled Presentation (.pptm), which are macro-enabled presentation files created with Microsoft PowerPoint 2007 or higher versions.
...
Represents a PowerPoint Open XML Presentation Template (.potx), which represents Microsoft PowerPoint template presentations created with Microsoft PowerPoint 2007 and above.
...
Represents a PowerPoint Open XML Macro-Enabled Presentation Template (.potm), which are Microsoft PowerPoint template files with support for macros. POTM files are created with PowerPoint 2007 or above and contain default settings that can be used to create further presentation files.
...
Represents a PowerPoint Open XML Macro-Enabled Slide (.ppsm), which represents a macro-enabled slide show file format created with Microsoft PowerPoint 2007 or higher.
...
Represents an OpenDocument Presentation Template (.otp), which represents presentation template files created by applications in the OASIS OpenDocument standard format.
...
Represents an Excel Spreadsheet (.xls), which represents the Excel Binary File Format. Such files can be created by Microsoft Excel as well as other similar spreadsheet programs such as OpenOffice Calc or Apple Numbers.
...
Represents an Excel 2003 XML (SpreadsheetML) (.xml), which represents the Excel Binary File Format. Such files can be created by Microsoft Excel as well as other similar spreadsheet programs such as OpenOffice Calc or Apple Numbers.
Learn more about this file format [here](../https://wiki.fileformat.com/spreadsheet/xls).
...
Represents Apple Numbers (.numbers), which represents an Excel-like Binary File Format. Such files can be created by Apple Numbers application.
...
Represents a Microsoft Excel Open XML Spreadsheet (.xlsx), which is a well-known format for Microsoft Excel documents that was introduced by Microsoft with the release of Microsoft Office 2007.
...
Represents an Excel Open XML Macro-Enabled Spreadsheet (.xlsm), which is a type of spreadsheet file that supports macros.
...
Represents an Excel Binary Spreadsheet (.xlsb), which specifies the Excel Binary File Format. It is a collection of records and structures that specify Excel workbook content.
...
Represents a Comma Separated Values File (.csv), which represents plain text files that contain records of data with comma-separated values.
...
Represents a Tab Separated Values File (.tsv), which represents data separated with tabs in plain text format.
...
Represents an OpenDocument Spreadsheet (.ods), which stands for OpenDocument Spreadsheet Document format that is editable by the user. Data is stored inside ODF files in rows and columns.
...
Represents an OpenDocument Flat XML Spreadsheet (.fods).
It is a spreadsheet document format based on XML used by OpenOffice and LibreOffice.
...
Represents an OpenDocument Spreadsheet Template (.ots).
It is a template document format used by OpenOffice and LibreOffice for spreadsheets.
...
Represents a Microsoft Excel Add-in (.xlam).
It is a file format used by Microsoft Excel to create add-ins with custom functionality.
Learn more about this file format [here](../https://fileinfo.com/extension/xlam).
...
Represents a Microsoft Excel Macro-Enabled Template (.xltm), which represents files generated by Microsoft Excel as Macro-enabled template files. XLTM files are similar to XLTX in structure except that the latter doesn't support creating template files with macros.
...
Represents a Microsoft Excel Template (.xlt), which are template files created with Microsoft Excel, a spreadsheet application that comes as part of the Microsoft Office suite. Microsoft Office 97-2003 supported creating new XLT files as well as opening them.
...
Represents an Excel Open XML Spreadsheet Template (.xltx), which represents Microsoft Excel Templates based on the Office OpenXML file format specifications. It is used to create a standard template file that can be utilized to generate XLSX files with the same settings specified in the XLTX file.
...
Represents a StarOffice Calc Spreadsheet (.sxc).
It is a spreadsheet document created with StarOffice Calc, a spreadsheet program.
...
Represents a Microsoft Project File (.mpp), which is a Microsoft Project data file that stores information related to project management in an integrated manner.
...
Represents a Microsoft Project Template (.mpt), which contains basic information and structure along with document settings for creating .MPP files.
...
Represents a Microsoft Project Exchange file (.mpx), which is an ASCII file format for transferring project information between Microsoft Project (MSP) and other applications that support the MPX file format, such as Primavera Project Planner, Sciforma, and Timerline Precision Estimating.
...
Represents an ActionScript File (.as).
Learn more about this file format [here](../https://docs.fileformat.com/programming/as).
...
Represents an ActionScript File (.as3).
Learn more about this file format [here](../https://datatypes.net/open-as3-files).
...
Represents an Assembly Language Source Code File (.asm).
Learn more about this file format [here](../https://docs.fileformat.com/cad/asm).
...
Represents a DOS Batch File (.bat).
Learn more about this file format [here](../https://docs.fileformat.com/executable/bat).
...
Represents a C/C++ Source Code File (.c).
Learn more about this file format [here](../https://wiki.fileformat.com/programming/c).
...
Represents a C++ Source Code File (.cc).
Learn more about this file format [here](../https://docs.fileformat.com/programming/cc).
...
Represents a CMake File (.cmake).
Learn more about this file format [here](../https://wiki.fileformat.com/programming/cmake).
...
Represents a C++ Source Code File (.cpp).
Learn more about this file format [here](../https://wiki.fileformat.com/programming/cpp).
...
Represents a C# Source Code File (.cs), which is a source code file for the C# programming language. It was introduced by Microsoft for use with the .NET Framework.
...
Represents a Visual Basic Project Item File (.vb), which is a source code file created in the Visual Basic language. It was created by Microsoft for the development of .NET applications.
...
Represents a Cascading Style Sheet (.css) file.
Learn more about this file format [here](../https://wiki.fileformat.com/web/css).
...
Represents a C++ Source Code File (.cxx).
Learn more about this file format [here](../https://docs.fileformat.com/programming/cxx).
...
Represents a Patch File (.diff).
Learn more about this file format [here](../https://docs.fileformat.com/programming/diff).
...
Represents a Ruby ERB Script (.erb).
Learn more about this file format [here](../https://docs.fileformat.com/programming/erb).
...
Represents a Groovy Source Code File (.groovy).
Learn more about this file format [here](../https://wiki.fileformat.com/programming/groovy).
...
Represents a C/C++/Objective-C Header File (.h).
Learn more about this file format [here](../https://docs.fileformat.com/programming/h).
...
Represents a Haml Source Code File (.haml).
Learn more about this file format [here](../https://wiki.fileformat.com/programming/haml).
...
Represents a C++ Header File (.hh).
Learn more about this file format [here](../https://docs.fileformat.com/programming/hh).
...
Represents a Java Source Code File (.java).
Learn more about this file format [here](../https://wiki.fileformat.com/programming/java).
...
Represents a JavaScript File (.js).
Learn more about this file format [here](../https://docs.fileformat.com/web/js).
...
Represents a JavaScript Object Notation File (.json).
...
Represents a LESS Style Sheet (.less) file.
Learn more about this file format [here](../https://wiki.fileformat.com/web/less).
...
Represents an Objective-C Implementation File (.m).
Learn more about this file format [here](../https://fileinfo.com/extension/m).
...
Represents an Xcode Makefile Script (.make).
Learn more about this file format [here](../https://docs.fileformat.com/programming/make).
...
Represents a Markdown Documentation File (.md).
Learn more about this file format [here](../https://docs.fileformat.com/word-processing/md).
...
Represents an ML Source Code File (.ml).
Learn more about this file format [here](../https://wiki.fileformat.com/programming/ml).
...
Represents an Objective-C++ Source File (.mm).
Learn more about this file format [here](../https://docs.fileformat.com/programming/mm).
...
Represents a PHP Source Code File (.php).
Learn more about this file format [here](../https://wiki.fileformat.com/programming/php).
...
Represents a Perl Script (.pl).
Learn more about this file format [here](../https://docs.fileformat.com/programming/pl).
...
Represents a Java Properties File (.properties).
Learn more about this file format [here](../https://fileinfo.com/extension/properties).
...
Learn more about this file format [here](../https://docs.fileformat.com/programming/py).
...
Represents a Ruby Source Code (.rb) file.
Learn more about this file format [here](../https://docs.fileformat.com/programming/rb).
...
Represents a reStructuredText File (.rst).
Learn more about this file format [here](../https://docs.fileformat.com/programming/rst).
...
Represents a Syntactically Awesome StyleSheets File (.sass).
Learn more about this file format [here](../https://docs.fileformat.com/web/sass).
...
Represents a Scala Source Code File (.scala).
Learn more about this file format [here](../https://docs.fileformat.com/programming/scala).
...
Represents a Scheme Source Code File (.scm).
Learn more about this file format [here](../https://docs.fileformat.com/programming/scm).
...
Represents a Generic Script File (.script).
Learn more about this file format [here](../https://docs.fileformat.com/programming/script).
...
Represents a Bash Shell Script (.sh).
Learn more about this file format [here](../https://docs.fileformat.com/cs/programming/sh).
...
Represents a Standard ML Source Code File (.sml).
Learn more about this file format [here](../https://fileinfo.com/extension/sml).
...
Represents a Structured Query Language Data File (.sql).
Learn more about this file format [here](../https://wiki.fileformat.com/database/sql).
...
Represents a Vim Settings File (.vim).
Learn more about this file format [here](../https://fileinfo.com/extension/vim).
...
Represents a YAML Document (.yaml).
Learn more about this file format [here](../https://docs.fileformat.com/programming/yaml).
...
Represents a Microsoft Word Document (.doc) file.
...
Represents a Microsoft Word Open XML Document (.docx) file.
...
Represents a Microsoft Compiled HTML Help File (.chm).
...
Represents a Word Open XML Macro-Enabled Document (.docm) file.
...
Represents a Word Document Template (.dot) file.
...
Represents a Word Open XML Document Template (.dotx) file.
...
Represents a Word Open XML Macro-Enabled Document Template (.dotm) file.
...
Represents a Rich Text Format File (.rtf).
...
Represents an OpenDocument Text Document (.odt).
...
Represents an OpenDocument Document Template (.ott).
It is a template document generated in compliance with the OASIS' OpenDocument standard format.
...
Represents a vCard File (.vcf), which is a digital file format for storing contact information.
It is widely used for data interchange among popular information exchange applications.
...
Represents an Adobe Illustrator file (.ai), which is a file format for Adobe Illustrator drawings.
Learn more about this file format [here](../https://docs.fileformat.com/image/ai).
...
Represents a PowerShell script module file (.psm1).
It is a file format for PowerShell module scripts.
...
Represents a PowerShell script file (.ps1).
It is a file format for Windows PowerShell Cmdlet files.
...
Represents a PowerShell script module manifest file (.psd1).
It is a file format for PowerShell module manifest scripts.
...
This method takes a file extension, with or without the period ".", and returns the corresponding file type if it is supported.
If the file type is not supported, it returns the default [UNKNOWN](../../com.groupdocs.viewer/filetype#UNKNOWN) file type.
...
If the file type is supported, the method returns the corresponding 
FileType
,
otherwise it returns the default [UNKNOWN](../../com.groupdocs.viewer/filetype#UNKNOWN) file type.
...
This method takes a file media type as input and maps it to the corresponding 
FileType
. If the media type is
supported, the method returns the corresponding file type. If the media type is not supported, it returns the default
[UNKNOWN](../../com.groupdocs.viewer/filetype#UNKNOWN) file type.
...
This method takes an input stream representing a file and tries to detect its file type by analyzing the file signature. The
file signature is a unique sequence of bytes at the beginning of the file that indicates its file type. The method
reads the file signature from the input stream and matches it against known file signatures of supported file types.
If a match is found, the method returns the corresponding file type. If no match is found, it returns the default
[UNKNOWN](../../com.groupdocs.viewer/filetype#UNKNOWN) file type.
...
This method analyzes the file signature, which is a unique sequence of bytes at the beginning of the file, to determine
the file type. It reads the file signature from the provided input stream and matches it against known file signatures
of supported file types. If a match is found, the method returns the corresponding file type. If no match is found, it
returns the default [UNKNOWN](../../com.groupdocs.viewer/filetype#UNKNOWN) file type.


If the file is encrypted and requires a password to open, the password should be provided as a parameter to this
method. The password will be used to decrypt the file and analyze its signature for determining the file type.
...
A logger can be provided to log any relevant information or errors during the file type detection process. The logger
should implement the ILogger interface. If no logger is provided, the detection process will not produce any
log output.
...
A password can be provided if the file is encrypted or protected. The password will be used to open the file before
analyzing the file signature. If the file does not require a password, this parameter can be set to null.
...
This method returns a sequence of all the supported file types by the system. The file types represent various
document formats that can be processed or recognized by the system.
***Learn more**
* Learn more about the supported document formats: [Full list of supported file formats](../https://docs.groupdocs.com/viewer/java/supported-document-formats/)*
