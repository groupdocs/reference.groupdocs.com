...
description: バーコード署名オプションを表します
type: docs
weight: 1260
url: /ja/net/groupdocs.signature.options/barcodesignoptions/
---
...
| 名前 | 説明 |
| --- | --- |
| [BarcodeSignOptions](barcodesignoptions#constructor)() | BarcodeSignOptions クラスの新しいインスタンスをデフォルト値で初期化します。 |
| [BarcodeSignOptions](barcodesignoptions#constructor_1)(string) | BarcodeSignOptions クラスの新しいインスタンスをテキストで初期化します。 |
| [BarcodeSignOptions](barcodesignoptions#constructor_2)(string, BarcodeType) | BarcodeSignOptions クラスの新しいインスタンスをテキストで初期化します。 |
...
| [CodeTextAlignment](../../groupdocs.signature.options/barcodesignoptions/codetextalignment) { get; set; } | 結果のバーコード イメージ内のテキストの配置を取得または設定します。 デフォルト値は [なし] です。 |
...
| [EncodeType](../../groupdocs.signature.options/barcodesignoptions/encodetype) { get; set; } | バーコード タイプを取得または設定します。 |
...
| override [ForeColor](../../groupdocs.signature.options/barcodesignoptions/forecolor) { get; set; } | バーコード バーの前の色を取得または設定します このプロパティを使用すると、検証で問題が発生する可能性があります。慎重に使用してください。 |
...
| [InnerMargins](../../groupdocs.signature.options/barcodesignoptions/innermargins) { get; set; } | バーコード要素と結果画像の境界線の間のスペースを取得または設定します。 |
...
| [ReturnContent](../../groupdocs.signature.options/barcodesignoptions/returncontent) { get; set; } | ドキュメント ページに配置された署名のバーコード イメージ コンテンツを取得するためのフラグを取得または設定します。[`ReturnContentType`](./returncontenttype) . デフォルトでは、このオプションは無効になっています. |
| [ReturnContentType](../../groupdocs.signature.options/barcodesignoptions/returncontenttype) { get; set; } | ReturnContent プロパティが有効な場合に返されるバーコード署名のイメージ コンテンツのファイル タイプを指定します。 デフォルトでは Null に設定されています。これは、バーコード イメージ コンテンツを元の形式で返すことを意味します。 この画像フォーマットは[`Format`](../../groupdocs.signature.domain/barcodesignature/format) サポートされている可能性のある値は、FileType.JPEG、FileType.PNG、FileType.BMP です。 提供された形式がサポートされていない場合、.png 形式のバーコード イメージ コンテンツが返されます。 |
...
* GroupDocs.Signatureによるバーコード電子署名作成の基本的な使い方: [バーコード署名でドキュメントに電子署名する方法](https://docs.groupdocs.com/display/signaturenet/eSign+document+with+Barcode+signature)
* GroupDocs.Signature: によるバーコード電子署名の設定の高度な使用法[バーコード署名と追加設定を使用してドキュメントを電子署名する高度な使用法](https://docs.groupdocs.com/display/signaturenet/Sign+document+with+Barcode+signature+and+additional+settings)
