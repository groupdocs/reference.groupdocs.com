...
description: 画像署名のプロパティが含まれています.
type: docs
weight: 590
url: /ja/net/groupdocs.signature.domain/imagesignature/
---
...
| 名前 | 説明 |
| --- | --- |
| [ImageSignature](imagesignature)(string) | 検索プロセス後に取得した署名識別子で ImageSignature オブジェクトを初期化します。 この一意の識別子は、ドキュメント署名情報レイヤーからこの署名の追加プロパティを見つけるために使用されます。 |

## プロパティ

| 名前 | 説明 |
| --- | --- |
| [Content](../../groupdocs.signature.domain/imagesignature/content) { get; } | タイプのイメージ バイナリ データ コンテンツを指定します[`Format`](./format). デフォルトでは、このプロパティは設定されません. プロパティを使用します[`ReturnContent`](../../groupdocs.signature.options/imagesearchoptions/returncontent)この機能を有効にするには. |
...
| [Format](../../groupdocs.signature.domain/imagesignature/format) { get; } | 署名画像の形式を指定します。 |
...
| [Size](../../groupdocs.signature.domain/imagesignature/size) { get; } | 署名画像のサイズをバイト単位で指定します。 |
...
| 名前 | 説明 |
| --- | --- |
| override [Clone](../../groupdocs.signature.domain/imagesignature/clone)() | バーコード署名インスタンスの複製. |
| override [Equals](../../groupdocs.signature.domain/imagesignature/equals)(object) | Equals メソッドを上書きして署名プロパティを比較します |
| override [GetHashCode](../../groupdocs.signature.domain/imagesignature/gethashcode)() | GetHashCode method をオーバーライドします |
