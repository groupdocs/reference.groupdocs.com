...
| 姓名 | 描述 |
| --- | --- |
| [BarcodeSignature](barcodesignature)(string) | 使用搜索过程后获得的签名标识符初始化 BarcodeSignature 对象。 此唯一标识符用于从文档签名信息层中查找此签名的其他属性。 |

## 特性

| 姓名 | 描述 |
| --- | --- |
| [Content](../../groupdocs.signature.domain/barcodesignature/content) { get; } | 指定条形码二进制数据图像内容类型[`Format`](./format). 默认情况下不会设置此属性。 使用属性[`ReturnContent`](../../groupdocs.signature.options/barcodesearchoptions/returncontent)启用此功能。 |
...
| [EncodeType](../../groupdocs.signature.domain/barcodesignature/encodetype) { get; } | 指定条码编码类型。 |
...
| [Text](../../groupdocs.signature.domain/barcodesignature/text) { get; } | 指定条码的文本。 |
