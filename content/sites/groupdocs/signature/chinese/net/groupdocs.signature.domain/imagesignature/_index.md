...
description: 包含图像签名属性
type: docs
weight: 570
url: /zh/net/groupdocs.signature.domain/imagesignature/
---
...
| 姓名 | 描述 |
| --- | --- |
| [ImageSignature](imagesignature)(string) | 使用搜索过程后获得的签名标识符初始化 ImageSignature 对象。 此唯一标识符用于从文档签名信息层查找此签名的附加属性。 |

## 特性

| 姓名 | 描述 |
| --- | --- |
| [Content](../../groupdocs.signature.domain/imagesignature/content) { get; } | 指定类型的图像二进制数据内容[`Format`](./format). 默认情况下不会设置此属性。 使用属性[`ReturnContent`](../../groupdocs.signature.options/imagesearchoptions/returncontent)启用此功能。 |
...
| [Format](../../groupdocs.signature.domain/imagesignature/format) { get; } | 指定签名图像的格式。 |
...
| [Size](../../groupdocs.signature.domain/imagesignature/size) { get; } | 指定签名图像的大小（以字节为单位）。 |
...
| 姓名 | 描述 |
| --- | --- |
| override [Clone](../../groupdocs.signature.domain/imagesignature/clone)() | 克隆条码签名实例。 |
| override [Equals](../../groupdocs.signature.domain/imagesignature/equals)(object) | 覆盖 Equals 方法以比较签名属性 |
| override [GetHashCode](../../groupdocs.signature.domain/imagesignature/gethashcode)() | 覆盖 GetHashCode 方法 |
