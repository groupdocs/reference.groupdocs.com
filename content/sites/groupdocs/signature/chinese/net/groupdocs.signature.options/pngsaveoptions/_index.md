---
title: PngSaveOptions
second_title: GroupDocs.Signature for .NET API 参考
description: 图像文档的 Png 保存选项
type: docs
weight: 1560
url: /zh/net/groupdocs.signature.options/pngsaveoptions/
---
## PngSaveOptions class

图像文档的 Png 保存选项。

```csharp
public sealed class PngSaveOptions : ImageSaveOptions
```

## 构造函数

| 姓名 | 描述 |
| --- | --- |
| [PngSaveOptions](pngsaveoptions)() | 使用默认值创建 PngSaveOptions. |

## 特性

| 姓名 | 描述 |
| --- | --- |
| [AddMissingExtenstion](../../groupdocs.signature.options/saveoptions/addmissingextenstion) { get; set; } | 获取或设置标志以在输出文件路径中缺少扩展名时自动添加扩展名 path 默认值为 false. |
| [BitDepth](../../groupdocs.signature.options/pngsaveoptions/bitdepth) { get; set; } | 位深度。 |
| [ColorType](../../groupdocs.signature.options/pngsaveoptions/colortype) { get; set; } | 获取或设置类型[`PngColorType`](../pngcolortype). |
| [CompressionLevel](../../groupdocs.signature.options/pngsaveoptions/compressionlevel) { get; set; } | 0-9范围内的png图像压缩级别，其中9是最大压缩 ，0是存储模式。 |
| [FileFormat](../../groupdocs.signature.options/imagesaveoptions/fileformat) { get; set; } | 获取或设置签名文档的文件格式。 |
| [FilterType](../../groupdocs.signature.options/pngsaveoptions/filtertype) { get; set; } | 获取或设置过滤器类型[`PngFilterType`](../pngfiltertype)在 png 文件保存过程中使用。 |
| [OverwriteExistingFiles](../../groupdocs.signature.options/saveoptions/overwriteexistingfiles) { get; set; } | 获取或设置是否用新的输出文件覆盖现有文件。 否则将创建以数字为后缀的新文件。 默认情况下，此值设置为 true，表示文件将被覆盖。 |
| [Password](../../groupdocs.signature.options/saveoptions/password) { get; set; } | 获取或设置密码以保存带密码保护的签名文档。 Image 文档不支持此属性。 |
| [Progressive](../../groupdocs.signature.options/pngsaveoptions/progressive) { get; set; } | 获取或设置一个值，指示此 PngSaveOptions 是否渐进。 |
| [UseOriginalPassword](../../groupdocs.signature.options/saveoptions/useoriginalpassword) { get; set; } | 获取或设置是否使用来自 LoadOptions 的密码将签名文档保存为受保护。 默认值为 true。 图像文档不支持此属性。 |

### 也可以看看

* class [ImageSaveOptions](../imagesaveoptions)
* 命名空间 [GroupDocs.Signature.Options](../../groupdocs.signature.options)
* 部件 [GroupDocs.Signature](../../)

<!-- DO NOT EDIT: generated by xmldocmd for GroupDocs.Signature.dll -->
