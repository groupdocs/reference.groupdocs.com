...
| [BarcodeSignOptions](barcodesignoptions#constructor_1)(string) | 使用文本初始化 BarcodeSignOptions 类的新实例。 |
| [BarcodeSignOptions](barcodesignoptions#constructor_2)(string, BarcodeType) | 使用文本初始化 BarcodeSignOptions 类的新实例。 |
...
| [CodeTextAlignment](../../groupdocs.signature.options/barcodesignoptions/codetextalignment) { get; set; } | 获取或设置结果条码图像中文本的对齐方式。 默认值为无。 |
...
| override [ForeColor](../../groupdocs.signature.options/barcodesignoptions/forecolor) { get; set; } | 获取或设置条码条的前景色 使用此属性可能会导致验证问题。小心使用它。 |
...
| [InnerMargins](../../groupdocs.signature.options/barcodesignoptions/innermargins) { get; set; } | 获取或设置条形码元素和结果图像边框之间的空间。 |
...
| [ReturnContent](../../groupdocs.signature.options/barcodesignoptions/returncontent) { get; set; } | 获取或设置标志以获取放在文档页面上的签名的条码图像内容。 如果此标志设置为真，条码签名图像内容将按要求的格式保存原始图像数据[`ReturnContentType`](./returncontenttype). 默认情况下禁用此选项。 |
| [ReturnContentType](../../groupdocs.signature.options/barcodesignoptions/returncontenttype) { get; set; } | 指定启用 ReturnContent 属性时条码签名的返回图像内容的文件类型。 默认设置为 Null。这意味着以原始格式返回条形码图像内容。 此图像格式指定于[`Format`](../../groupdocs.signature.domain/barcodesignature/format) 可能支持的值为：FileType.JPEG、FileType.PNG、FileType.BMP。 如果不支持提供的格式，则将返回 .png 格式的条形码图像内容。 |
...
* GroupDocs.Signature 创建 Barcode 电子签名的基本用法： [如何使用条形码签名对文档进行电子签名](https://docs.groupdocs.com/display/signaturenet/eSign+document+with+Barcode+signature)
* 使用 GroupDocs.Signature 设置条码电子签名的高级用法： [带有条形码签名和其他设置的电子签名文档的高级用法](https://docs.groupdocs.com/display/signaturenet/Sign+document+with+Barcode+signature+and+additional+settings)
