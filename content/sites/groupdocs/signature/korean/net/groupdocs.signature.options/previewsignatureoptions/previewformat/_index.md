---
title: PreviewFormat
second_title: .NET API 참조용 GroupDocs.Signature
description: 미리보기 이미지 형식을 가져오거나 설정합니다. 기본값은 PNG 입니다.
type: docs
weight: 20
url: /ko/net/groupdocs.signature.options/previewsignatureoptions/previewformat/
---
## PreviewSignatureOptions.PreviewFormat property

미리보기 이미지 형식을 가져오거나 설정합니다. 기본값은 PNG 입니다.

```csharp
public PreviewFormats PreviewFormat { get; set; }
```

### 또한보십시오

* enum [PreviewFormats](../../previewsignatureoptions.previewformats)
* class [PreviewSignatureOptions](../../previewsignatureoptions)
* 네임스페이스 [GroupDocs.Signature.Options](../../previewsignatureoptions)
* 집회 [GroupDocs.Signature](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for GroupDocs.Signature.dll -->
