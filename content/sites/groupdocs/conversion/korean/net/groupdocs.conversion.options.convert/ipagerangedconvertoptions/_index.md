---
title: IPageRangedConvertOptions
second_title: .NET API 참조용 GroupDocs.Conversion
description: 특정 페이지 목록의 변환을 지원하는 변환 옵션을 나타냅니다
type: docs
weight: 1580
url: /ko/net/groupdocs.conversion.options.convert/ipagerangedconvertoptions/
---
## IPageRangedConvertOptions interface

특정 페이지 목록의 변환을 지원하는 변환 옵션을 나타냅니다

```csharp
public interface IPageRangedConvertOptions : IConvertOptions
```

## 속성

| 이름 | 설명 |
| --- | --- |
| [Pages](../../groupdocs.conversion.options.convert/ipagerangedconvertoptions/pages) { get; set; } | 변환할 페이지 인덱스 목록입니다. 특정 페이지를 변환하려면 지정해야 합니다. |

### 또한보십시오

* interface [IConvertOptions](../iconvertoptions)
* 네임스페이스 [GroupDocs.Conversion.Options.Convert](../../groupdocs.conversion.options.convert)
* 집회 [GroupDocs.Conversion](../../)

<!-- DO NOT EDIT: generated by xmldocmd for GroupDocs.Conversion.dll -->
