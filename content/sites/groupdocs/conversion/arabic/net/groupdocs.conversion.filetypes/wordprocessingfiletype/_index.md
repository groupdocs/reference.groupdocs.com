...
يحدد ملفات معالجة الكلمات التي تحتوي على معلومات المستخدم بتنسيق نص عادي أو نص منسق. يحتوي تنسيق ملف نص عادي على نص غير منسق ولا يمكن تطبيق أي إعدادات للخط أو الصفحة وما إلى ذلك. في المقابل ، يسمح تنسيق ملف النص المنسق بخيارات التنسيق مثل تعيين نوع الخطوط والأنماط (غامق ومائل وتسطير ، وما إلى ذلك) ، وهوامش الصفحة والعناوين والتعداد النقطي والأرقام والعديد من ميزات التنسيق الأخرى. يتضمن أنواع الملفات التالية: [`Doc`](./doc) ، [`Docm`](./docm) ، [`Docx`](./docx) ، [`Dot`](./dot) ، [`Dotm`](./dotm) ، [`Dotx`](./dotx) ، Mobi ، [`Odt`](./odt) ، [`Ott`](./ott) ، [`Rtf`](./rtf) ، [`Txt`](./txt) . [`Md`](./md) . Log . تعرف على المزيد حول تنسيقات معالجة الكلمات[هنا](https://wiki.fileformat.com/word-processing) .
