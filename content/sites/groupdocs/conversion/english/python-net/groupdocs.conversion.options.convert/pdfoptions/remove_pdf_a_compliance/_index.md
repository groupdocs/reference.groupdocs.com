---
title: remove_pdf_a_compliance property
second_title: GroupDocs.Conversion for Python via .NET API References
description: "The property removes PDF/A compliance."
type: docs
url: /python-net/groupdocs.conversion.options.convert/pdfoptions/remove_pdf_a_compliance/
is_root: false
weight: 2070
---


## remove_pdf_a_compliance property

The property removes PDF/A compliance.

### Definition:
```python
@property
def remove_pdf_a_compliance(self):
    ...
@remove_pdf_a_compliance.setter
def remove_pdf_a_compliance(self, value):
    ...
```

### See Also
* class [`PdfOptions`](/conversion/python-net/groupdocs.conversion.options.convert/pdfoptions/)
