...
SEARCH_LABELS.set('reference.aspose.com', 'asposeapiref');
SEARCH_LABELS.set('reference2.aspose.com', 'asposeapiref');
function clearSearchResult(searchResults, pfName, sdk) {
    return searchResults.filter((el) => 
        !el.host.includes("blog.aspose.com") 
        && !el.host.includes("docs.aspose.com")
...
function determineSearchLabel() {
   return SEARCH_LABELS.has(window.location.hostname) ? SEARCH_LABELS.get(window.location.hostname) : SEARCH_LABELS.get('reference.aspose.com');
}

function getSearchResults(query, pfName, sdk) {
    var label = determineSearchLabel();
    var domain = "search.aspose.com";
