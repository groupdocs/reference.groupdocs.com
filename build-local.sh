...
#   python scripts/serve-local.py 1313
set -euo pipefail
...
for p in "${PRODUCTS[@]}"; do
  ...
  python scripts/resolve_md_links.py "$OUT/$p" --base-url "http://localhost:$PORT/$p" >/dev/null
  bash scripts/move_md_to_ugly_urls.sh "$OUT/$p" >/dev/null
...
python scripts/build_search_index.py "$OUT" --out "$OUT/search-index.json"
...
python scripts/build_llms_full.py --source content/sites/groupdocs --base-url "http://localhost:$PORT" \
  --only "$(IFS=,; echo "${PRODUCTS[*]}")" --out "$OUT/llms-full.txt"

echo
echo "Built ${#PRODUCTS[@]} product(s) + home into ./$OUT"
echo "Serve with:  python scripts/serve-local.py $PORT      # UTF-8 charset, matches production"
